...
配置数值参考 model-infer-parallel-analysis skill 的 `{file:./references/config-index.md}`。
...
> TODO: CP 见 `models/deepseek-v3.2-exp/`，KVP 见 `models/longcat-flash/`。
...
参考实现：`models/deepseek_r1/utils/convert_model.py`
...
| 实现模式 | 参考文件 | 搜索关键词 |
|---------|---------|-----------|
| TP 线性层替换 | `models/gpt_oss/models/modeling_*.py` | `QKVParallelLinear` |
| 通信组创建 | `models/kimi-k2-thinking/models/modeling_*.py` | `init_parallel_comm_group` |
| 通信组校验 | `models/kimi-k2-thinking/infer.py` | `check_parallel_settings` |
| MoE EP Prefill | `models/deepseek_r1/models/modeling_*.py` | `npu_moe_re_routing` |
| MoE EP Decode | `models/deepseek_r1/models/modeling_*.py` | `npu_moe_distribute_dispatch` |
| Embed/LMHead 并行 | `models/kimi-k2-thinking/models/modeling_*.py` | `VocabParallelEmbedding` |
| oproj_tp 独立配置 | `models/longcat-flash/models/modeling_*.py` | `oproj_tp` |
