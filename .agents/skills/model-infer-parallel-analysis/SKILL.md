...
> **CP / KVP 实施支持**：本 skill 输出可包含 `cp_size` / `kvp_size` 候选，但 `model-infer-parallel-impl` skill 当前不直接支持这两个维度的代码实施，需参照仓内已有模型手动改造（CP 参考 `models/deepseek-v3.2-exp/`，KVP 参考 `models/longcat-flash/`）。
