...
输出目录默认 `models/{model_name}/`，可通过 dispatch prompt 配置其他路径。
...
1. **复制 HF 实现到 models/ 目录**：从 HF transformers 复制 modeling/configuration 相关文件，最小改动适配框架。**禁止 `from transformers` 导入模型类：modeling 和 configuration 代码必须 vendoring 到仓库内，不依赖 transformers 提供模型实现**：
...
| 模型架构 | 参考模型 | 参考路径 |
|---------|---------|---------|
| 标准 LLM（MHA/GQA） | gpt-oss | `models/gpt_oss/` |
| MoE 架构 | qwen3-moe | `models/qwen3_moe/` |
| MLA 架构（DeepSeek 系列） | deepseek-r1 | `models/deepseek_r1/` |
| 长序列 | longcat-flash | `models/longcat-flash/` |
| 多模态/视频生成 | hunyuan-video | `models/hunyuan-video/` |
...
| ModelRunner 基类 | `executor/model_runner.py` |
