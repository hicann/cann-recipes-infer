...
- `torchair.CompilerConfig()` + `torchair.get_npu_backend()` + GE 图模式：走 Ascend IR 路径。先看 `docs/zh/ascend_ir/features/advanced/multi_stream.md`，需要控核时再看 `docs/zh/ascend_ir/features/advanced/limit_cores.md`。
- `torch.compile(..., backend="npugraph_ex")`：走 npugraph_ex / aclgraph 路径。先看 `docs/zh/npugraph_ex/advanced/multi_stream.md`，需要控核时再看 `docs/zh/npugraph_ex/advanced/limit_cores.md`。
