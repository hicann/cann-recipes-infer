...
public:
    ...
    __aicore__ inline void InitLocalBuffers()
    {
        ...
        uint32_t totalBufferSize =
            inQSize * 2 +  // Q和K队列（每个都是双缓冲）
            inVSize +      // V队列（双缓冲）
            inStateSize +  // State输入队列（双缓冲）
            inGamaSize +   // Gamma队列
            inBetaSize +   // Beta队列
            outStateSize + // State输出队列（双缓冲）
            outAttnSize;
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint64_t workIdx = startIdx; workIdx < endIdx; workIdx++) {
            ...
            // 每个batch只有1个token，seq0 = batchIdx, seq1 = batchIdx + 1
