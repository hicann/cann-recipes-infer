...
description: KVCache 优化技能。分析和优化 LLM/多模态模型推理中的 KVCache 实现，包括连续缓存、分页注意力（Paged Attention）配合 FA 融合算子、MLA 压缩缓存。触发场景包括：KVCache 管理实现、paged attention、KV 压缩、FA 融合算子、OOM/性能问题、block_table/slot_mapping 构造。基于本仓库已有模型的 KVCache 实现经验，按模型类型和场景推荐最佳方案。
user-invocable: true
---
...
**参考文件**：`models/qwen3-moe/models/modeling_qwen3_moe.py`（BSH）、`models/gpt-oss/models/modeling_gpt_oss.py`（TND）
...
**参考文件**：`models/deepseek-r1/models/modeling_deepseek.py`、`models/deepseek-v3.2-exp/models/modeling_deepseek.py`
...
**参考文件**：`models/deepseek-r1/models/modeling_deepseek.py`（`forward_absorb()`）
...
```python
# 参考: models/deepseek-v3.2-exp/models/modeling_deepseek.py (line 809-820)
num_blocks_per_seq = max_seq_len // block_size
total_blocks = batch_size * num_blocks_per_seq
...
```python
# 参考: models/deepseek-v3.2-exp/models/modeling_deepseek.py (line 2275-2282)
all_tensors = []
...
```python
# 参考: models/deepseek-v3.2-exp/models/modeling_deepseek.py (line 2283-2284)
# 预计算偏移（一次性）
...
```python
# 参考: models/deepseek-v3.2-exp/models/modeling_deepseek.py (line 2286-2301)
# TND Prefill 示例:
...
**kv_len 是 Runner 层变量，各层只读不写。** `kv_len` 作为参数传入所有 Transformer 层，`scatter_update_` 使用同一个 `kv_len` 写入每层各自的缓存。不应在 attention/cache 内部递增 kv_len——否则 Prefill 阶段各层写入位置逐层偏移，精度损坏。参考 `models/qwen3-moe/models/modeling_qwen3_moe.py`。
...
- **shape**：`sparse_mode=3/4` 时固定 `[2048, 2048]`，与 `max_position_embeddings` 无关。本仓库统一用 `get_init_attn_mask(2048, device)` 构造（见 `executor/utils/common_utils.py` 的 `get_init_attn_mask` 函数）
...
if q_len > 1:  # Prefill（或 MTP Decode）
    ...
    atten_mask = None
```
参考：`models/deepseek-r1/models/modeling_deepseek.py` 的 Prefill/Decode 分支逻辑
...
- **KV 并行（KVP）**：KVCache 沿 head 维度切分到多卡。参考 `models/longcat-flash/`
- **KVCache 量化**：INT8/W8A8C8 量化。参考 `models/deepseek-v3.2-exp/models/offload_cache.py`
- **CPU-GPU Offload**：`torch_npu.empty_with_swapped_memory` + 异步双流。参考 `models/hstu/modules/gpu_kv_cache_manager.py`
- **扩散模型缓存（DiT Cache）**：TeaCache/FBCache/TaylorSeer。参考 `module/dit_cache/cache_method.py`
