...
description: torch_npu 融合算子自动分析与优化技能。分析 PyTorch 模型代码，识别可优化的计算模式，推荐并生成 torch_npu 融合算子替换方案。触发场景包括：NPU/昇腾推理性能优化、torch_npu 融合算子替换、MoE/量化推理/Attention 等模块的 NPU 适配。基于项目内算子接口文档和仓库已有模型的融合算子使用经验，按计算语义推荐最佳替换方案。
user-invocable: true
---
...
**注意**：若 FA 融合算子已在前置阶段替换，仅跳过 FA 调用本身。Attention 的其他子链路（RoPE、KV Cache 写入、Residual+Norm、QKV/O Projection 等）仍需逐一检查是否有可用融合算子。
...
- `KV Cache` 静态化 / `PA` 改造：参考 `kvcache-optimization` skill
...
| MoE 算子模式详解 | [`references/moe-patterns.md`](references/moe-patterns.md) |
