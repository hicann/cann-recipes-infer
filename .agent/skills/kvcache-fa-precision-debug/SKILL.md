---
name: kvcache-fa-precision-debug
description: KVCache 与 FA 精度调试技能。诊断 KVCache 写入和 Flash Attention 融合算子引入的精度问题，提供系统化的排查流程和常见问题修复方案。触发场景包括：KVCache/FA 替换后精度验证未通过、模型输出与基线存在显著偏差、Prefill 和 Decode 精度表现不一致、出现 NaN/Inf、量化模式下精度异常放大等。
user-invocable: true
---

# KVCache 与 FA 精度调试技能

按"症状分类 → 快速验证 → 分模块定位 → 逐层对比 → 陷阱修复"的分层策略排查精度问题。完整校验工具见 `references/fa_debug_utils.py`。
...
| actual_seq_lengths 错误 | TND 应用 cumsum，BSH 用原始值 | 参考 `kvcache-optimization` skill 的数据流章节 |
...
> 表格供快速验证清单引用，完整校验代码见 `references/fa_debug_utils.py`，可在 FA 调用前插入 `debug_fa_all_params()` 一键检查。
