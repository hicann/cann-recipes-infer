...
| 路径 | 先读文档 | 再读文档 | 适用场景 |
| --- | --- | --- | --- |
| Ascend IR / GE 图模式 | `docs/zh/ascend_ir/features/advanced/multi_stream.md` | `docs/zh/ascend_ir/features/advanced/limit_cores.md` | 图内多流表达、`npu_stream_switch`、`npu_wait_tensor`、GE 图模式控核 |
| npugraph_ex / aclgraph | `docs/zh/npugraph_ex/advanced/multi_stream.md` | `docs/zh/npugraph_ex/advanced/limit_cores.md` | `torch.npu.Stream` / `Event` / `record_stream`、Stream 级控核 |
...
| --- | --- | --- | --- |
| eager / patch 改造 | 显式流对象 | `torch.npu.Stream()`、`record_event()`、`wait_event()`、`wait_stream()`、`record_stream()` | 先看仓库案例；如果要对齐显式 stream 语义，参考 `docs/zh/npugraph_ex/advanced/multi_stream.md` |
| `ge_graph` / TorchAir 图内多流 | 图内 scope | `npu_stream_switch`、`npu_wait_tensor` | 先看 `docs/zh/ascend_ir/features/advanced/multi_stream.md`；需要控核时再看 `docs/zh/ascend_ir/features/advanced/limit_cores.md` |
| `npugraph_ex` / aclgraph | 显式 stream + Event | `torch.npu.Stream()`、`torch.npu.stream()`、`torch.npu.Event()`、`record_stream()` | 先看 `docs/zh/npugraph_ex/advanced/multi_stream.md`；需要控核时再看 `docs/zh/npugraph_ex/advanced/limit_cores.md` |
...
| 需要延长 tensor 生命周期 | `record_stream()` | 短生命周期 tensor 会在别的流继续使用 | 主要看 aclgraph / eager / capture 阶段；权重等长生命周期对象一般不需要 | `docs/zh/npugraph_ex/advanced/multi_stream.md` |
