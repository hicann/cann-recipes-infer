...
### 提交PR，完善交付内容
当方案共识达成后，您可以提交对应的PR，并记得关联到之前的RFC。提交PR时需注意：
- 在正式提交PR前，新增模型样例请逐项对照[新模型合入 Checklist](docs/common/new_model_checklist.md)进行验证和检查，确保上库时新增模型满足Checklist中的所有要求。
- 为确保样例质量和可复用性，PR中辛苦您至少包含以下内容：
...
适用于基于裸模型的优化样例，样例需要合入到 `integration/custom/` 目录下，您可以参考以下结构组织您的代码和文档（样例名建议全部小写，多个单词之间用短中划线连接，比如qwen3-moe）：

```
├── integration/                                # 开发者贡献样例目录
|  ├── custom/                                  
