...
Router target 列表（由 prefill node-0 的 shell 计算后传给 `--role router`）：
...
Prefill 侧 **node 0**（`VC_TASK_INDEX=0`）额外起一个 Router 进程（`function.sh` 中 `PD_ROLE=prefill && VC_TASK_INDEX=0` 分支），与该节点的 prefill 父进程同机运行。Router 监听 `ROUTER_HTTP_PORT`（8000），处理客户端请求时：
...
worker_main(global_rank, local_rank, ..., disagg_config)              # in server.py
  └── OnlineInference.__init__
        ├── 父类 OfflineInference.__init__                              # 加载模型 + HCCL 组 + KV cache
        ├── comm_manager.init_cpu_groups()                              # 追加 Gloo 调度域：tp_cpu_group / dp_leader_group
        ...
        └── kv_transfer_mgr.register_memory(kvcache_manager)
...
flowchart LR
    ...
    subgraph RP[Router 进程<br/>prefill-node-0 同机]
...
`BootstrapState`（`executor/online/bootstrap.py`）是 Bootstrap server 的内存数据结构，包含两类字段：启动时从 Prefill YAML 读入并固定的拓扑参数；运行时由 prefill rank 注册或 per-request 调用写入的注册表。
...
文件：`executor/online/router.py`。`server.py` 在 `--role router` 模式下调用 `create_router_app(prefill_addrs, decode_addrs, bootstrap_ports)` 并用 uvicorn 暴露在 `ROUTER_HTTP_PORT`（8000）。Router 进程只起在 prefill node-0 上。

### 4.2 路由规则

按请求级 `bootstrap_room` 哈希取模选目标：

- prefill：`bootstrap_room % len(prefill_targets)`（在 `Router._inject_pd_fields` 中内联，无独立选择函数）
...
def _inject_pd_fields(request_dict):
    ...
    target_idx = bootstrap_room % len(self.prefill_targets)   # 内联，无独立选择函数
...
# bootstrap 注册延后到 register_memory()——由 OnlineInference 在 __init__ 返回后调用
```
...
`_register_to_bootstrap` 由 `register_memory()` 在 `KVTransferManager.__init__` 返回后调用（`OnlineInference.__init__` 中触发），内部首先按 `disaggregation_mode` 分支：
...
| 字典 | 来源 | 内容 |
|---|---|---|
| `prefill_info_table: dict[bootstrap_addr, PrefillServerInfo]` | bootstrap `/route` 直接返回 | 原始拓扑：`attn_tp_size / attn_cp_size / dp_size / block_size（Optional） / kv_cache_dtype（Optional） / ranks` |
| `target_rank_map_table: dict[bootstrap_addr, TargetRankMapping]` | decode 本地推导 | 我要向哪几个 prefill rank 拉 KV：`target_tp_rank / target_tp_ranks / target_cp_ranks / required_dst_info_num / required_prefill_response_num` |
...
仅 `--engine_backend mooncake` 路径需要。具体参考[hixl wiki](https://gitcode.com/cann/hixl/wiki/MooncakeTE-Hixl使用说明.md)：

```bash
git clone -b v0.3.8 --depth 1 https://github.com/kvcache-ai/Mooncake.git
cd Mooncake && git submodule update --init --recursive
bash dependencies.sh  # CentOs 需要将该脚本中的 apt 安装换成 yum 安装（包名也需对应调整）
source /usr/local/Ascend/ascend-toolkit/set_env.sh  # 按实际安装路径调整
mkdir build && cd build
cmake .. -DUSE_ASCEND_DIRECT=ON
make -j
make install  # 该过程会将 *.so 拷贝到系统路径，请主要运行时该路径需要在 `LD_LIBRARY_PATH` 中
```

### 7.5 `server.py` CLI

该方式主要在 `function.sh` 中调用（不需要手动执行，手动执行入口为 `infer.sh`）。

```bash
# Prefill / Decode 服务父进程
python3 server.py \
    --role prefill \
    --yaml-file-path <prefill.yaml> \
...
`--ips` 是**本角色**（Prefill或Decode）的节点 IP 列表，按 `--node-index` 索引取出本节点 IP 注入 `DisaggConfig.local_ip`（在 `server.py:main()` 中完成）；这是 **rank_ip 的唯一来源**，下文 `KVTransferManager` 通告的 `rank_ip` 即此值。跨实例拓扑列表分别由 `--prefill-ips` / `--decode-ips` 提供（由 `function.sh` 计算后传入）。
