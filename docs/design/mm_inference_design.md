...
├── config/
│   ├── {scenario}.yaml             # 多卡配置
│   ├── {scenario}_fbcache.yaml     # 单卡 + FBCache
│   ├── {scenario}_teacache.yaml    # 单卡 + TeaCache
│   ├── {scenario}_taylorseer.yaml  # 单卡 + TaylorSeer
│   └── {scenario}_*.yaml           # 其他变体（量化、分辨率等）
├── {Python 入口脚本}               # 由 YAML 的 entry_script 指向
├── res/{YYYYMMDD}/{model_name}/    # 拉起时自动创建的日志目录
└── {模型代码}                       # 保持不变

executor/scripts/
└── mm_function.sh                  # 统一拉起函数库
...
bash infer.sh
  └── mm_launch()
        ...
        ├── mm_generate_cache_config()   # 检测 dit_cache 段，拼装 --cache-config / --cache_config
        └── mm_launch_task()             # 根据 launcher 调用 torchrun 或 accelerate
```

`infer.sh` 本身非常简洁（四个模型完全相同），仅做三件事：source `mm_function.sh`；导出 `MODEL_DIR`、`YAML`；调用 `mm_launch`：
...
**`model_args` 不是白名单**：任何被入口脚本 `argparse` 接收的参数都可以通过 `model_args` 传入，包括 DiT 权重路径（`dit-weight`）、量化开关（`use-fp8` / `fa-perblock-fp8` / `mm-mxfp8`）、profiler 开关（`prof-dit`）、UAA 开关（`ulysses-anything`）等。示例仅展示常用参数，不代表全部可用参数。
...
3. **HCCL 通信参数**（最高优先级，硬编码在 `mm_function.sh`）：
...
4. **用户自行 export**：`ASCEND_RT_VISIBLE_DEVICES` 等设备选择类变量由用户在执行 `bash infer.sh` 前手动 export，`mm_function.sh` 不处理。

### 4.4 `launcher_args` 使用范围

`launcher_args` 仅在 `launcher: "accelerate"` 时生效。每对 `k: v` 转为 `--k=v`（等号分隔，区别于 `model_args` 的空格分隔），拼入 `accelerate launch` 的启动参数中。常用键包括 `mixed_precision`、`config_file` 等，完整列表参考 [accelerate launch 文档](https://huggingface.co/docs/accelerate/package_reference/cli)。
...
Dit-Cache 与序列并行正交。HunyuanVideo 同时预置了多卡 + Dit-Cache 的 YAML（`sp8_fbcache.yaml` / `sp8_teacache.yaml` / `sp8_taylorseer.yaml`）；Wan2.2-I2V 的多卡 YAML 固定 `dit_cache.method: NoCache`。如需派生新的多卡 + Dit-Cache 组合，复制对应多卡 YAML 并修改 `dit_cache.method` / `params` 即可。

## 6. 各模型 YAML 清单

| 模型 | YAML 文件 | 卡数 | 启动器 | Cache | 关键特性 |
|------|-----------|------|--------|-------|---------|
| Wan2.2-I2V | `14b_single.yaml` | 1 | torchrun | NoCache | 单卡基线，`832*480*61`，`convert_model_dtype` |
| | `14b_single_fbcache.yaml` | 1 | torchrun | FBCache | `832*480*61`，`convert_model_dtype` |
| | `14b_single_teacache.yaml` | 1 | torchrun | TeaCache | `832*480*61`，默认 `rel_l1_thresh=0.1` |
| | `14b_cfg2_ulysses4.yaml` | 8 | torchrun | NoCache | CFG=2 × Ulysses=4, VAE 并行，`1280*720*81` / `832*480*61`，DiT+T5 FSDP |
| HunyuanVideo | `single.yaml` | 1 | torchrun | NoCache | 单卡基线，`720*1280*129`，CPU offload |
| | `single_fbcache.yaml` | 1 | torchrun | FBCache | `720*1280*129`，CPU offload |
| | `single_teacache.yaml` | 1 | torchrun | TeaCache | `rel_l1_thresh=0.15`，CPU offload |
| | `single_taylorseer.yaml` | 1 | torchrun | TaylorSeer | CPU offload（需 400GB+ 主机内存） |
| | `single_fp8.yaml` | 1 | torchrun | NoCache | FP8 FA 激活量化（需 950PR） |
| | `sp8.yaml` | 8 | torchrun | NoCache | Ulysses=8, VAE 并行，`720*1280*129` |
| | `sp8_fbcache.yaml` | 8 | torchrun | FBCache | 8 卡 SP + FBCache |
| | `sp8_teacache.yaml` | 8 | torchrun | TeaCache | 8 卡 SP + TeaCache |
| | `sp8_taylorseer.yaml` | 8 | torchrun | TaylorSeer | 8 卡 SP + TaylorSeer |
| SANA-Video | `2b_480p_single.yaml` | 1 | accelerate | — | `mixed_precision=bf16`，`DISABLE_XFORMERS=1`，2B 480p |
...
- 调整模型权重路径：修改 `model_args.ckpt_dir` / `model-id` / `dit-weight` / `model_path`
