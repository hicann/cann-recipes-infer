...
```
cann-recipes-infer/
├── AGENTS.md              # 项目根指令（Claude Code / OpenCode 共用）
├── docs/agent/            # Agent 体系设计文档
├── scripts/init-agent.sh  # 初始化脚本
└── .agents/               # Skills / SubAgents / Hooks 源目录
    ├── README.md          # Skill 体系总览
    ├── agents/            # SubAgent 定义（3 个）
...
Hook 脚本位于 `.agents/hooks/`，在工具调用时自动触发：
