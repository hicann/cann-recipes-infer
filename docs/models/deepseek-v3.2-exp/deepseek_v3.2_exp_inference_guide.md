...
本文主要介绍基于A3集群的DeepSeek-V3.2-Exp模型的Prefill和Decode推理优化，首先分析了DeepSeek-V3.2-Exp模型的结构特点，实现了长序列亲和模型并行策略，并设计了新的NPU融合Kernel和多流并行优化。基于这些优化点，本实践0day实现了DeepSeek-V3.2-Exp BF16/Int8模型部署，未来将持续完成低比特量化/KVCache压缩算法。4~8K常规序列继承原有DeepSeek-V3.1优化，16K~166K长序列推理场景结合DSA稀疏收益，性能超越DeepSeek-V3.1，模型和融合Kernel均已开源。
...
大EP场景下，MoE消耗单卡显存较多。除了MLAProlog的KVCache，Lightning Indexer额外增加了KCache。在长序列情况下，对整个推理集群的计算访存比提出了新的挑战。相对于BF16推理，Int8量化可以有效降低端到端时延，提升系统吞吐量。
