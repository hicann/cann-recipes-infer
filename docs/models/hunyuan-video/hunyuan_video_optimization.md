...
通过运行命令 python sample_video.py --sparse_method TopK/SVG 使能Sparse Attention，通过修改`models/hunyuan-video/hyvideo/sparse/sparse_config.yaml`
```python
comment: choose from TopK/SVG 
block_size_Q: 128 # 设置q, k分块的大小
block_size_K: 512
model: HunyuanVideo
TopK:
   predictor_name: TopK
   sparse_time_step: "10-49" # 应用稀疏的step
   sparsity_files_path: ./sparsity/320x480x65/v3 # 稀疏度文件路径
   CAC_threshold: 0.9 # CAC阈值
SVG:
   predictor_name: SVG
   sparse_time_step: "14-49" # 应用稀疏的step
   sparsity: 0.65 # 稀疏度
