...
MXFP8量化注意力通过将注意力计算中的Q、K、V张量量化为MXFP8格式（`npu_dynamic_mx_quant`），显著降低显存带宽需求，特别适用于显存受限或带宽受限的场景。

#### 量化原理

本方案采用MXFP8（Micro-scaling FP8）动态量化策略：

1. **per-token/per-channel group MX量化**：对 Q/K/V 分别做动态 MX 量化，每个块组独立计算缩放因子，缩放因子采用 E8M0 格式（纯指数表示），仅占1字节。

2. **差异化量化模式**：
   - **Query/Key**: 沿 hidden dim 方向分组，即FIA_v2算子入参 `quant_mode=6`。
   - **Value**: 沿 seq_len 方向分组，即FIA_v2算子入参 `quant_mode=8`。
...
def npu_mxfp8_quant(x, dst_type=torch.float8_e4m3fn, output_layout="TND", axis=-1):
    ...
    if output_layout == "TND":
        x_tnd = x.contiguous().view(b * s, n, d)
        x_quant, x_scale = torch_npu.npu_dynamic_mx_quant(
            x_tnd, dst_type=dst_type, axis=axis
        )
...
def npu_mxfp8_attn(q, k, v, dst_type=torch.float8_e4m3fn, softmax_scale=None):
    ...
    return attn_out.contiguous().view(b, s, n, d)
```
...
output = npu_mxfp8_attn(q, k, v, dst_type=torch.float8_e4m3fn)
```
...
attn = UnifiedSPAttention(
    ulysses_group=ulysses_group,
    ring_group=None,
    fa_mxfp8=True  # 启用MXFP8量化注意力
)
```

#### 量化模式说明

MXFP8使用 `npu_dynamic_mx_quant` 算子进行动态 MX 量化，`npu_fused_infer_attention_score_v2` 算子内部处理以下流程：
1. 使用 E8M0 缩放因子对量化值进行反量化
2. 执行注意力计算（Q×K^T）
3. 应用Softmax
4. 执行输出计算（Attn×V）
5. 返回原始精度格式的输出
...
- MXFP8量化注意力当前仅支持Ulysses序列并行，不支持Ring Attention
