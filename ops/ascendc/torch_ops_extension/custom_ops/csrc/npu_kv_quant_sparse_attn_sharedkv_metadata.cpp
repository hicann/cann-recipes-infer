    ...
    const c10::string_view device)
{
    ...
    // 调用EXEC_NPU_CMD_V1
    EXEC_NPU_CMD_V1(aclnnKvQuantSparseAttnSharedkvMetadata, cu_seqlens_q_val, cu_seqlens_ori_kv_val, cu_seqlens_cmp_kv_val, 
...
TORCH_LIBRARY_IMPL(custom, PrivateUse1, m) {
    m.impl("npu_kv_quant_sparse_attn_sharedkv_metadata", &custom::npu_kv_quant_sparse_attn_sharedkv_metadata_npu);
}


// step5, 为META设备注册前向实现
TORCH_LIBRARY_IMPL(custom, Meta, m) {
    m.impl("npu_kv_quant_sparse_attn_sharedkv_metadata", &custom::npu_kv_quant_sparse_attn_sharedkv_metadata_meta);
}
