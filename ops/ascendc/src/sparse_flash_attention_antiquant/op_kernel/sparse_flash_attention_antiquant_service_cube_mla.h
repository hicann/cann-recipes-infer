...
/*!
 * \file sparse_flash_attention_antiquant_service_cube_mla.h
 * \brief use 7 buffer for matmul l1, better pipeline
 */
#ifndef SPARSE_FLASH_ATTENTION_ANTIQUANT_SERVICE_CUBE_MLA_H
#define SPARSE_FLASH_ATTENTION_ANTIQUANT_SERVICE_CUBE_MLA_H
...
*/
template <typename T, SFAA_LAYOUT SRC_LAYOUT>
                                  ...
                                  const Position &startPos)   // bacthIdx nIdx curSeqIdx
{
    ...
    while (copyFinishRowCnt < shape.copyRowNum) {
        ...
        if constexpr (SRC_LAYOUT == SFAA_LAYOUT::BSND || SRC_LAYOUT == SFAA_LAYOUT::TND) {
...
template <typename SFAAT> class SFAAMatmulService {
public:
    ...
    __aicore__ inline SFAAMatmulService(){};
...
template <typename SFAAT> __aicore__ inline void SFAAMatmulService<SFAAT>::InitParams(const ConstInfo &constInfo)
{
...
SFAAMatmulService<SFAAT>::InitMm1GlobalTensor(GlobalTensor<Q_T> queryGm, GlobalTensor<Q_T> qRopeGm,
                                              GlobalTensor<KV_T> keyGm, GlobalTensor<K_ROPE_T> kRopeGm,
                                              GlobalTensor<MM_OUT_T> mm1ResGm)
{
    // mm1
...
template <typename SFAAT>
__aicore__ inline void
SFAAMatmulService<SFAAT>::InitMm2GlobalTensor(GlobalTensor<K_ROPE_T> vec1ResGm, GlobalTensor<KV_T> valueGm,
                                              GlobalTensor<MM_OUT_T> mm2ResGm, GlobalTensor<OUT_T> attentionOutGm)
{
    // mm2
...
template <typename SFAAT>
__aicore__ inline void
SFAAMatmulService<SFAAT>::InitPageAttentionInfo(const GlobalTensor<K_ROPE_T>& kvMergeGm,
                                                GlobalTensor<int32_t> blockTableGm, GlobalTensor<int32_t> topKGm,
                                                uint32_t blockSize, uint32_t maxBlockNumPerBatch)
{
...
template <typename SFAAT> __aicore__ inline void SFAAMatmulService<SFAAT>::InitBuffers(TPipe *pipe)
{
...
template <typename SFAAT> __aicore__ inline void SFAAMatmulService<SFAAT>::UpdateKey(GlobalTensor<KV_T> keyGm)
{
    this->keyGm = keyGm;
}

template <typename SFAAT> __aicore__ inline void SFAAMatmulService<SFAAT>::UpdateValue(GlobalTensor<KV_T> valueGm)
{
    this->valueGm = valueGm;
}

template <typename SFAAT> __aicore__ inline void SFAAMatmulService<SFAAT>::AllocEventID()
{
...
template <typename SFAAT> __aicore__ inline void SFAAMatmulService<SFAAT>::FreeEventID()
{
...
template <typename SFAAT>
__aicore__ inline void SFAAMatmulService<SFAAT>::CopyGmToL1(LocalTensor<K_ROPE_T> &l1Tensor,
...
template <typename SFAAT>
__aicore__ inline void SFAAMatmulService<SFAAT>::CopyInMm1AToL1(LocalTensor<K_ROPE_T> &l1Tensor, const RunInfo &info,
...
template <typename SFAAT>
__aicore__ inline void SFAAMatmulService<SFAAT>::CopyInMm1ARopeToL1(LocalTensor<K_ROPE_T> &l1Tensor,
...
template <typename SFAAT>
__aicore__ inline void
SFAAMatmulService<SFAAT>::CopyInMm1BToL1(LocalTensor<K_ROPE_T> &bL1Tensor, const uint64_t keyGmBaseOffset,
                                         uint32_t copyTotalRowCntAlign, uint32_t copyStartRowCnt,
                                         uint32_t nActCopyRowCount, uint32_t headSize)
{
    uint64_t dStride = constInfo.headDim;
    if constexpr (LAYOUT_T == SFAA_LAYOUT::BSND || LAYOUT_T == SFAA_LAYOUT::TND) {
...
template <typename SFAAT>
__aicore__ inline void
SFAAMatmulService<SFAAT>::CopyInMm1BRopeToL1(LocalTensor<K_ROPE_T> &bL1Tensor, const uint64_t kRopeGmBaseOffset,
                                             uint32_t copyTotalRowCntAlign, uint32_t copyStartRowCnt,
                                             uint32_t nActCopyRowCount, uint32_t headSize)
{
...
template <typename SFAAT>
__aicore__ inline void SFAAMatmulService<SFAAT>::LoadDataMm1A(LocalTensor<K_ROPE_T> &aL0Tensor,
...
template <typename SFAAT>
__aicore__ inline void SFAAMatmulService<SFAAT>::LoadDataMm1B(LocalTensor<K_ROPE_T> &l0Tensor,
...
template <typename SFAAT>
__aicore__ inline void SFAAMatmulService<SFAAT>::CopyInMm2AToL1(LocalTensor<K_ROPE_T> &aL1Tensor, const RunInfo &info,
...
template <typename SFAAT>
__aicore__ inline void SFAAMatmulService<SFAAT>::CopyInMm2BToL1(
...
template <typename SFAAT>
__aicore__ inline void SFAAMatmulService<SFAAT>::CalcTopKBlockInfo(
...
template <typename SFAAT>
__aicore__ inline void SFAAMatmulService<SFAAT>::ComputeMm1(const RunInfo &info, const MSplitInfo mSplitInfo)
{
    // 最外层还需要一层m的循环
    uint32_t mSize = mSplitInfo.nBufferDealM;
    uint32_t mL1Size = M_SPLIT_SIZE;
    uint32_t mL1SizeAlign = SFAAAlign(M_SPLIT_SIZE, 16U);
    ...
    uint32_t nL1SizeAlign = SFAAAlign(N_SPLIT_SIZE, 16U);
    ...
    for (uint32_t mL1 = 0; mL1 < mL1Loops; mL1++) {
        ...
        mL1SizeAlign = SFAAAlign(M_SPLIT_SIZE, 16U);
        if (mL1 == (mL1Loops - 1)) {
            // 尾块重新计算size
            mL1Size = mSize - (mL1Loops - 1) * M_SPLIT_SIZE;
            mL1SizeAlign = SFAAAlign(mL1Size, 16U);
        }
        ...
        for (uint32_t nL1 = 0; nL1 < nL1Loops; nL1++) { // L1切n, 512/128=4
            if (nL1 == (nL1Loops - 1)) {
                ...
                nL1SizeAlign = SFAAAlign(nL1Size, 16U);
            }
...
template <typename SFAAT>
__aicore__ inline void SFAAMatmulService<SFAAT>::ComputeMm2(const RunInfo &info, const MSplitInfo mSplitInfo)
{
    uint32_t mSize = mSplitInfo.nBufferDealM;
    ...
    uint32_t kL1SizeAlign = SFAAAlign(kL1Size, 16U);
    ...
    for (uint32_t nL1 = 0; nL1 < nL1Loops; nL1++) { // n切L1
        ...
        // k l1写成一个循环, 和mm1保持一致
        kL1Size = 256;
        kL1SizeAlign = SFAAAlign(kL1Size, 16U);
...
#endif // SPARSE_FLASH_ATTENTION_ANTIQUANT_SERVICE_CUBE_MLA_H