...
/*!
 * \file sparse_flash_attention_antiquant_kernel_mla.h
 * \brief
 */

#ifndef SPARSE_FLASH_ATTENTION_ANTIQUANT_KERNEL_MLA_H
#define SPARSE_FLASH_ATTENTION_ANTIQUANT_KERNEL_MLA_H
...
#include "sparse_flash_attention_antiquant_service_cube_mla.h"
#include "sparse_flash_attention_antiquant_service_vector_mla.h"
...
template <typename SFAAT> class SparseFlashAttentionAntiquantMla {
public:
    ...
    __aicore__ inline SparseFlashAttentionAntiquantMla(){};
...
private:
    ...
    static constexpr uint32_t BLOCK_ELEMENT_NUM = SFAAVectorService<SFAAT>::BYTE_BLOCK / sizeof(T);
    ...
    SFAAMatmulService<SFAAT> matmulService;
    SFAAVectorService<SFAAT> vectorService;
...
template <typename SFAAT> __aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::InitTilingData()
{
...
template <typename SFAAT> __aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::InitBuffers()
{
    if ASCEND_IS_AIV {
...
SparseFlashAttentionAntiquantMla<SFAAT>::InitActualSeqLen(__gm__ uint8_t *actualSeqLengthsQ,
...
template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::InitAllZeroOutput(uint32_t bIdx, uint32_t s1Idx,
                                                                                  uint32_t n2Idx)
{
...
template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::InitOutputSingleCore()
{
...
template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::GetActualSeqLen(uint32_t bIdx, uint32_t s1Idx)
{
...
template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::GetSparseActualSeqLen(uint32_t bIdx, uint32_t s1Idx,
                                                                                      uint32_t n2Idx)
{
    ...
    if constexpr (TEMPLATE_MODE == V_TEMPLATE) {
        ...
        if constexpr (LAYOUT_T == SFAA_LAYOUT::BSND) { // B,S1,N2 K
...
template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::DealActSeqLenIsZero(uint32_t bIdx, uint32_t s1Idx,
                                                                                    uint32_t n2Idx)
{
    if ASCEND_IS_AIV {
...
template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::GetPreNextTokensLeftUp()
{
    if (constInfo.sparseMode == 3) {
...
template <typename SFAAT> __aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::UpdateInnerLoopCond()
{
...
template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::UpdateInner(uint32_t &s2End, uint32_t &curS2End,
...
template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::Init(__gm__ uint8_t *query,
...
template <typename SFAAT> __aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::InitCalcParamsEach()
{
    // 计算总的基本块
...
template <typename SFAAT>
__aicore__ inline void
SparseFlashAttentionAntiquantMla<SFAAT>::Bmm2DataCopyOut(uint64_t attenOutOffset, LocalTensor<OUT_T> &attenOutUb,
                                                         ...
                                                         uint32_t columnCount, uint32_t actualColumnCount)
{
    ...
    dataCopyParams.srcStride = (columnCount - actualColumnCount) / (SFAAVectorService<SFAAT>::BYTE_BLOCK /
        sizeof(OUT_T));
...
template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::CalcParams(uint32_t loop, uint64_t s2Start,
                                                                           uint32_t s2LoopIdx, RunInfo &info)
{
    ...
    info.actualSingleProcessSInnerSizeAlign =
        SFAAAlign((uint32_t)info.actualSingleProcessSInnerSize, (uint32_t)SFAAVectorService<SFAAT>::BYTE_BLOCK);
}

template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::ComputeMm1(const RunInfo &info)
{
...
template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::ComputeMm2(const RunInfo &info)
{
    uint32_t nBufferLoopTimes = (info.actMBaseSize + constInfo.nBufferMBaseSize - 1) / constInfo.nBufferMBaseSize;
    uint32_t nBufferTail = info.actMBaseSize - (nBufferLoopTimes - 1) * constInfo.nBufferMBaseSize;
    for (uint32_t i = 0; i < nBufferLoopTimes; i++) {
...
template <typename SFAAT> __aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::Process()
{
    if (aiCoreIdx < usedCoreNum) {
        if ASCEND_IS_AIV {
...
template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::GetBN2Idx(uint32_t bN2Idx, uint32_t &bIdx,
                                                                          uint32_t &n2Idx)
{
...
template <typename SFAAT> __aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::ProcessBalance()
{
    RunInfo extraInfo[SFAA_PRELOAD_TASK_CACHE_SIZE];
    uint32_t gloop = 0;
    int gS1LoopEnd;
...
template <typename SFAAT>
__aicore__ inline void
SparseFlashAttentionAntiquantMla<SFAAT>::PreloadPipeline(uint32_t loop, uint64_t s2Start, uint64_t s2LoopIdx,
                                                         RunInfo extraInfo[SFAA_PRELOAD_TASK_CACHE_SIZE])
{
...
SparseFlashAttentionAntiquantMla<SFAAT>::GetBalanceActualSeqLengths(GlobalTensor<int32_t> &actualSeqLengths,
                                                                    uint32_t bIdx)
{
    if constexpr (LAYOUT_T == SFAA_LAYOUT::TND) {
        if (bIdx > 0) {
...
template <typename SFAAT>
__aicore__ inline void SparseFlashAttentionAntiquantMla<SFAAT>::GetAxisStartIdx(uint32_t bN2EndPrev,
                                                                                uint32_t s1GEndPrev,
                                                                                uint32_t s2EndPrev)
{
...
#endif // SPARSE_FLASH_ATTENTION_ANTIQUANT_KERNEL_MLA_H