...
/*!
 * \file rms_norm_dynamic_quant_cut_d_kernel.h
 * \brief
 */

#ifndef RMS_NORM_DYNAMIC_QUANT_SLICE_D_H_
#define RMS_NORM_DYNAMIC_QUANT_SLICE_D_H_
...
private:
    ...
    __aicore__ inline void CopyInTmpX(int32_t baseGmOffset, int32_t rowGmOffset, int32_t elementCount, float rstdLocalTemp)
    {
        ...
        DataCopyEx(xLocalIn, this->xGm[baseGmOffset + rowGmOffset], elementCount);
        inRowsQue.EnQue(xLocalIn);
        LocalTensor<T> xLocal = inRowsQue.template DeQue<T>();
        Cast(yLocalFp32, xLocal, RoundMode::CAST_NONE, elementCount);
        PipeBarrier<PIPE_V>();
        Muls(yLocalFp32, yLocalFp32, rstdLocalTemp, elementCount);
        PipeBarrier<PIPE_V>();
...
#endif // RMS_NORM_DYNAMIC_QUANT_SLICE_D_H_
