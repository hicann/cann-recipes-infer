...
#define INIT_AND_PROCESS                                                                                \
    ...
    if (TILING_KEY_IS(0)) {
        ...
        KernelRmsNormDynamicQuantNormal<DTYPE_X, DTYPE_Y1, 1> op(&pipe);
        INIT_AND_PROCESS;
    } else if (TILING_KEY_IS(2)) {
        KernelRmsNormDynamicQuantSingleRow<DTYPE_X, DTYPE_Y1, 2> op(&pipe);
        INIT_AND_PROCESS;
    } else if (TILING_KEY_IS(3)) {
        KernelRmsNormDynamicQuantSliceD<DTYPE_X, DTYPE_Y1, 3> op(&pipe);
        INIT_AND_PROCESS;
    }
}