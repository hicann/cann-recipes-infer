...
bool QuantRopeRegBaseTilingClassABAAndBA::IsCapable()
{
    // BNSD对应11SD和B1SD两种brc模式
    return (IsRegbaseSocVersion()) && (layout_ == RopeLayout::BNSD);
}

ge::graphStatus QuantRopeRegBaseTilingClassABAAndBA::SplitCore()
{
    ...
    if (blockNumB1 == 0) {
        OPS_LOG_I("QuantRopeRegBaseTilingClassABAAndBA SplitCore error, blockNumB1 == 0");
    ...
    if (blockNumS2 == 0) {
        OPS_LOG_I("QuantRopeRegBaseTilingClassABAAndBA SplitCore error, blockNumS2 == 0");
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus QuantRopeRegBaseTilingClassABAAndBA::ComputeUbFactor()
{
    ubFactorS_ = 1;
    ubFactorB_ = 1;
    ubFactorN_ = 1;
    int64_t dSize = CeilAlign(d_ * GetSizeByDataType(dtype_) / dSplitCoef_, this->blockSize_) * dSplitCoef_;
    int64_t dSizeH8 = CeilAlign(d_ / dSplitCoef_, this->blockSize_) * dSplitCoef_;
    int64_t numOfDAvailable = FloorDiv(static_cast<int64_t>(aicoreParams_.ubSize),
        UB_FACTOR * dSize + DOUBLE_BUFFER * dSizeH8);
    ...
    if (ubFactorS_ == 0) {
        OPS_LOG_I("QuantRopeRegBaseTilingClassABAAndBA ComputeUbFactor error, ubFactorS_ == 0");
        return ge::GRAPH_FAILED;
    }
    ...
    if (cosb_ == 1) {
        ...
        if (ubFactorN_ == 0) {
            OPS_LOG_I("QuantRopeRegBaseTilingClassABAAndBA ComputeUbFactor error, ubFactorN_ == 0");
            return ge::GRAPH_FAILED;
        }
...
void QuantRopeRegBaseTilingClassABAAndBA::SetTilingData()
{
    ...
    OPS_LOG_I(context_->GetNodeName(),
        "QuantRopeRegBaseTilingClassABAAndBA tilingData: "
...
ge::graphStatus QuantRopeRegBaseTilingClassABAAndBA::DoOpTiling()
{
...
ge::graphStatus QuantRopeRegBaseTilingClassABAAndBA::DoLibApiTiling()
{
    return ge::GRAPH_SUCCESS;
}

uint64_t QuantRopeRegBaseTilingClassABAAndBA::GetTilingKey() const
{
    return cosb_ == 1 ? TILING_KEY_BA : TILING_KEY_ABA;
}

ge::graphStatus QuantRopeRegBaseTilingClassABAAndBA::GetWorkspaceSize()
{
    workspaceSize_ = WORKSPACE_SIZE;
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QuantRopeRegBaseTilingClassABAAndBA::PostTiling()
{
    SetTilingData();
