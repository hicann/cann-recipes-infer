...
ge::graphStatus QuantRopeRegBaseTilingClassAB::DoOpTiling()
{
    int64_t bs = b_ * s_;
    if (cosb_ == 1) {
        bs = s_;
    ...
    if (typeSize == 0) {
        OPS_LOG_I("QuantRopeRegBaseTilingClassAB DoOpTiling error, typeSize == 0");
    ...
    if (bs <= int64_t(aicoreParams_.blockDim) / CONST_TWO) {
        if (blockNumBS_ == 0) {
            OPS_LOG_I("QuantRopeRegBaseTilingClassAB ComputeUbFactor error, blockNumBS_ == 0");
            return ge::GRAPH_FAILED;
        }
    ...
    int64_t dAlignH8_ = CeilAlign(d_ / dSplitCoef_, blockSize_);
    int64_t baseBufferSize = dAlign_ * typeSize + dAlignH8_ * dSplitCoef_;
...
ge::graphStatus QuantRopeRegBaseTilingClassAB::PostTiling()
{
    ...
    OPS_LOG_I(context_->GetNodeName(),
        "QuantRopeRegBaseTilingClassAB tilingData is B: %ld, CosB: %ld, S: %ld, D: %ld, N: %ld, kAlign: %ld, "
...
uint64_t QuantRopeRegBaseTilingClassAB::GetTilingKey() const
{
    return TILING_KEY_AB;
}

bool QuantRopeRegBaseTilingClassAB::IsCapable()
{
