...
ge::graphStatus QuantRopeRegBaseTilingClassBAB::DoOpTiling()
{
...
ge::graphStatus QuantRopeRegBaseTilingClassBAB::SplitCore()
{
    // 尝试先对B分核，再尝试优先对S分核，比较二者切分之后的总核数
    auto blockFactorB1 = CeilDiv(b_, coreNum_);
    auto blockNumB1 = CeilDiv(b_, blockFactorB1);
    if (blockNumB1 == 0) {
        OPS_LOG_I("QuantRopeRegBaseTilingClassBAB SplitCore error, blockNumB1 == 0");
        return ge::GRAPH_FAILED;
    }
    ...
    if (blockNumS2 == 0) {
        OPS_LOG_I("QuantRopeRegBaseTilingClassBAB SplitCore error, blockNumS2 == 0");
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus QuantRopeRegBaseTilingClassBAB::SplitUb()
{
    uint32_t typeSize = ge::GetSizeByDataType(dtype_);
    int64_t dAlign = CeilAlign(d_ * typeSize / dSplitCoef_, blockSize_) * dSplitCoef_;
    int64_t dAlignH8 = CeilAlign(d_ / dSplitCoef_, blockSize_) * dSplitCoef_;
    int64_t canLoadDNum = FloorDiv(ubSize_, dAlign + dAlignH8);
    ...
    if (ubFactorN_ == 0) {
        OPS_LOG_I("QuantRopeRegBaseTilingClassBAB SplitUb error, ubFactorN_ == 0");
        return ge::GRAPH_FAILED;
    }
...
void QuantRopeRegBaseTilingClassBAB::PrintTilingData()
{
    OPS_LOG_I(context_->GetNodeName(),
        "QuantRopeRegBaseTilingClassBAB tilingData: useCoreNum is %ld,"
        ...
        tilingData_.get_ubFactorN(), tilingData_.get_ubTailFactorN(), tilingData_.get_rotaryMode(),
        tilingKey_, tilingData_.get_sliceStart(), tilingData_.get_sliceEnd(), tilingData_.get_sliceLength());
    return;
}

ge::graphStatus QuantRopeRegBaseTilingClassBAB::PostTiling()
{
...
uint64_t QuantRopeRegBaseTilingClassBAB::GetTilingKey() const
{
