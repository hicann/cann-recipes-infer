...
constexpr AscendC::MicroAPI::CastTrait castTraitF32toh8 = {
    AscendC::MicroAPI::RegLayout::ZERO,
...
__aicore__ inline void QuantizeToHifloat8(__local_mem__ hifloat8_t* yLocalAddr, MicroAPI::RegTensor<float>& xReg,
	MicroAPI::MaskReg& preg, uint32_t offset, float scale)
{
    MicroAPI::RegTensor<hifloat8_t> tmp;
    Muls(xReg, xReg, scale, preg);
    Cast<hifloat8_t, float, castTraitF32toh8>(tmp, xReg, preg);
    DataCopy<hifloat8_t, MicroAPI::StoreDist::DIST_PACK4_B32>(yLocalAddr + offset, tmp, preg);
}
    ...
    MicroAPI::MaskReg &preg, uint32_t offset)
{
    if constexpr (IsSameType<T, half>::value) {
        ...
        DataCopy<int32_t, AscendC::MicroAPI::StoreDist::DIST_NORM>(((__local_mem__ int32_t *)output + offset),
				zInt32, preg);
        ...
        DataCopy<int8_t, MicroAPI::StoreDist::DIST_PACK4_B32>(((__local_mem__ int8_t *)output + offset),
			zInt8, preg);
        ...
        DataCopy<uint8_t, MicroAPI::StoreDist::DIST_PACK4_B32>(((__local_mem__ uint8_t *)output + offset),
			zUint8, preg);
    } else {
