...
ge::graphStatus LIQInfoParser::CheckShapeDim()
{
    ...
    OPS_ERR_IF(outShapeDim != expectShapeDim,
        OPS_LOG_E(
            opName_, "the dim num of sparse_indices's shape should be %u, but now is %u", expectShapeDim, outShapeDim),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(!((weightsShapeDim == (expectShapeDim - 1)) ||
        ...
        OPS_LOG_E(
            opName_, "the dim num of weights's shape should be %u, but now is %u", expectShapeDim - 1, weightsShapeDim),
        return ge::GRAPH_FAILED);
...
ge::graphStatus LIQInfoParser::GetAndCheckN2Size()
{
    ...
    OPS_ERR_IF(n2Size_ != 1, OPS_LOG_E(opName_, "N2 only support 1."), return ge::GRAPH_FAILED);
...
ge::graphStatus LIQInfoParser::GetAndCheckBlockSize()
{
    ...
    OPS_ERR_IF(((blockSize_ % BLOCK_SIZE_FACTOR != 0) || (blockSize_ == 0) || (blockSize_ > BLOCK_SIZE_LIMIT)),
        OPS_LOG_E(opName_,
            "input key's block_size must be a multiple of 16 and belong to (0, 1024], but now is %u.",
            blockSize_),
        return ge::GRAPH_FAILED);
...
ge::graphStatus LIQInfoParser::ValidateInputShapesMatch()
{
    ...
    */
    ...
    if (qLayout_ == DataLayout::TND) {
        // -----------------------check BatchSize-------------------
        // bSize_ 来源于act_seq_q
        OPS_ERR_IF((opParamInfo_.actualSeqLengthsK.tensor->GetShapeSize() != bSize_) ||
        ...
        OPS_ERR_IF((opParamInfo_.weights.shape->GetStorageShape().GetDim(1) != s1Size_) ||
            ...
            OPS_LOG_E(opName_,
                "BSND case input query, weight, sparse_indices dim 1 are %u, %u, %u, they must be same.",
                s1Size_,
...
ge::graphStatus LIQInfoParser::CheckScaleShape()
{
    ...
    OPS_ERR_IF(qDequantScaleShapeDim != (qShapeDim - 1),
        OPS_LOG_E(opName_, "the dim num of query_dequant_scale's shape should be %u, but now is %u", qShapeDim - 1,
