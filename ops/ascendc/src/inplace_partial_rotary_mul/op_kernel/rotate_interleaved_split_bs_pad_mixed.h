...
/*!
 * \file rotate_interleaved_split_bs_pad_mixed.h
 * \brief
 */
#ifndef ROTATE_INTERLEAVED_SPLIT_BS_PAD_MIXED_H
#define ROTATE_INTERLEAVED_SPLIT_BS_PAD_MIXED_H
...
template <typename T>
class InterleavedSplitBSPadMixed {
public:
    __aicore__ inline InterleavedSplitBSPadMixed(){};
...
template <typename T>
__aicore__ inline void InterleavedSplitBSPadMixed<T>::InitData(const RopeRegbaseTilingData *tiling)
{
    tiling_ = tiling;
    ...
    uint32_t seqIdx, uint32_t calcLen)
{
    ...
    bsPadDataCopyTriParams.blockLen = headDim * sizeof(float);
...
template <typename T>
__aicore__ inline void InterleavedSplitBSPadMixed<T>::CopyOut(uint32_t seqIdx, uint32_t batchIdx, uint32_t calcLen)
{
...
template <typename T>
__aicore__ inline void InterleavedSplitBSPadMixed<T>::Process()
{
                                                                 ...
                                                                 uint32_t calcLen)
{
    ...
    Mul(tmp32Buf3, tmp32BsPadBuf1, cos, calcTotalNum);
    ...
    CopyInSin(cos, seqIdx, calcLen);
    Mul(tmp32BsPadBuf1, tmp32BsPadBuf1, cos, calcTotalNum);
    inQueCos.FreeTensor(cos);
...
#endif // ROTATE_INTERLEAVED_SPLIT_BS_PAD_MIXED_H
