...
/*!
 * \file rotate_interleaved_split_s_mixed.h
 * \brief
 */
#ifndef ROTATE_INTERLEAVED_SPLIT_S_MIXED_H
#define ROTATE_INTERLEAVED_SPLIT_S_MIXED_H
...
template <typename T>
class InterleavedSplitSMixed {
public:
    __aicore__ inline InterleavedSplitSMixed(){};
...
template <typename T>
__aicore__ inline void InterleavedSplitSMixed<T>::Init(GM_ADDR x, GM_ADDR cos, GM_ADDR sin, GM_ADDR y,
                                                  const RopeRegbaseTilingData *tiling, TPipe *pipe)
{
    InitData(tiling);
...
template <typename T>
__aicore__ inline void InterleavedSplitSMixed<T>::InitData(const RopeRegbaseTilingData *tiling)
{
    tiling_ = tiling;
...
template <typename T>
__aicore__ inline void InterleavedSplitSMixed<T>::CopyInX(LocalTensor<T> &x, uint32_t loopIdx, uint32_t calcLen)
{
...
template <typename T>
__aicore__ inline void InterleavedSplitSMixed<T>::CopyInCos(LocalTensor<float> &cos, uint32_t loopIdx, uint32_t calcLen)
{
    ...
    dataCopyTriParams.dstStride = static_cast<uint16_t>((numHeads - 1) * headDim * sizeof(float) / BLOCK_SIZE);
...
template <typename T>
__aicore__ inline void InterleavedSplitSMixed<T>::CopyInSin(LocalTensor<float> &sin, uint32_t loopIdx, uint32_t calcLen)
{
...
template <typename T>
__aicore__ inline void InterleavedSplitSMixed<T>::CopyOut(uint32_t loopIdx, uint32_t calcLen)
{
    DataCopyExtParams dataCopyParams;
    dataCopyParams.blockCount = calcLen*numHeads;
    dataCopyParams.blockLen = headDim * sizeof(T);
...
template <typename T>
__aicore__ inline void InterleavedSplitSMixed<T>::Process()
{
...
template <typename T>
__aicore__ inline void InterleavedSplitSMixed<T>::Compute(uint32_t loopIdx, LocalTensor<uint32_t> &gatherOffsetCast,
                                                     uint32_t calcLen)
{
...
template <typename T>
__aicore__ inline void InterleavedSplitSMixed<T>::ComputeCastFp32(uint32_t loopIdx,
    ...
    uint32_t calcLen)
{
    ...
    Mul(tmp32Buf3, tmp32Buf1, cos, calcTotalNum);
    ...
    CopyInSin(cos, loopIdx, calcLen);
    Mul(tmp32Buf1, tmp32Buf1, cos, calcTotalNum);
    inQueCos.FreeTensor(cos);
...
#endif // ROTATE_INTERLEAVED_SPLIT_S_MIXED_H
