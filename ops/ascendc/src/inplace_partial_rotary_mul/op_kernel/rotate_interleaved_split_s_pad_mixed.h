...
/*!
 * \file rotate_interleaved_split_s_pad_mixed.h
 * \brief
 */
#ifndef ROTATE_INTERLEAVED_SPLIT_S_PAD_MIXED_H
#define ROTATE_INTERLEAVED_SPLIT_S_PAD_MIXED_H
...
template <typename T>
class InterleavedSplitSPadMixed {
public:
    __aicore__ inline InterleavedSplitSPadMixed(){};
...
protected:
    ...
    uint32_t triBlockNum = BLOCK_SIZE / sizeof(float);
    ...
    __aicore__ inline void CopyInCos(LocalTensor<float> &cos, uint32_t loopIdx, uint32_t calcLen);
    __aicore__ inline void CopyInSin(LocalTensor<float> &sin, uint32_t loopIdx, uint32_t calcLen);
...
template <typename T>
__aicore__ inline void InterleavedSplitSPadMixed<T>::Init(GM_ADDR x, GM_ADDR cos, GM_ADDR sin, GM_ADDR y,
                                                     const RopeRegbaseTilingData *tiling, TPipe *pipe)
{
    InitData(tiling);
...
template <typename T>
__aicore__ inline void InterleavedSplitSPadMixed<T>::InitData(const RopeRegbaseTilingData *tiling)
{
    tiling_ = tiling;
...
template <typename T>
__aicore__ inline void InterleavedSplitSPadMixed<T>::CopyInX(LocalTensor<T> &x, uint32_t loopIdx, uint32_t calcLen)
{
    ...
    for (uint32_t batchIdx = 0; batchIdx < batchSize; ++batchIdx) {
        ...
        DataCopyPad(x[batchIdx * calcLen * numHeads * headDimAlign],
                    xGm[batchIdx * seqLen * bufferNdSizeAll +
                        loopIdx * ubCalcSeq * bufferNdSizeAll + start],
...
template <typename T>
__aicore__ inline void InterleavedSplitSPadMixed<T>::CopyInCos(LocalTensor<float> &cos,
    uint32_t loopIdx, uint32_t calcLen)
{
    DataCopyExtParams dataCopyTriParams;
    dataCopyTriParams.blockCount = calcLen;
    dataCopyTriParams.blockLen = headDim * sizeof(float);
    dataCopyTriParams.srcStride = 0;
    dataCopyTriParams.dstStride = static_cast<uint16_t>((numHeads - 1) * headDimAlign / triBlockNum);
...
template <typename T>
__aicore__ inline void InterleavedSplitSPadMixed<T>::CopyInSin(LocalTensor<float> &sin,
    uint32_t loopIdx, uint32_t calcLen)
{
...
template <typename T>
__aicore__ inline void InterleavedSplitSPadMixed<T>::CopyOut(uint32_t loopIdx, uint32_t calcLen)
{
...
template <typename T>
__aicore__ inline void InterleavedSplitSPadMixed<T>::Process()
{
...
template <typename T>
__aicore__ inline void InterleavedSplitSPadMixed<T>::Compute(uint32_t loopIdx, LocalTensor<uint32_t> &gatherOffsetCast,
                                                        uint32_t calcLen)
{
...
InterleavedSplitSPadMixed<T>::ComputeCastFp32(uint32_t loopIdx,
    LocalTensor<uint32_t> &gatherOffsetCast, uint32_t calcLen)
{
    ...
    Mul(tmp32Buf3, tmp32SPadBuf1, cos, totalCount);
    ...
    CopyInSin(cos, loopIdx, calcLen);
    Mul(tmp32SPadBuf1, tmp32SPadBuf1, cos, totalCount);
    inQueCos.FreeTensor(cos);
...
#endif // ROTATE_INTERLEAVED_SPLIT_S_PAD_MIXED_H
