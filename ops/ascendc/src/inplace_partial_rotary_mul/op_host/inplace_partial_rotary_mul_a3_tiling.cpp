...
ge::graphStatus InplacePartialRotaryMulTiling::CheckInput()
{
    ...
    OPS_ERR_IF(xInput == nullptr || inputR1 == nullptr || inputR2 == nullptr, OPS_LOG_E(context_->GetNodeName(), "get input nullptr."),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(yOutput == nullptr, OPS_LOG_E(context_->GetNodeName(), "get output nullptr."),
        return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(xDesc == nullptr || cosDesc == nullptr || sinDesc == nullptr || yDesc == nullptr,
        OPS_LOG_E(context_->GetNodeName(), "get input or output desc nullptr."),
        return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(!(xDtype_ == ge::DT_FLOAT16 || xDtype_ == ge::DT_BF16 || xDtype_ == ge::DT_FLOAT),
        OPS_LOG_E(context_->GetNodeName(), "x dtype only support float16, bfloat16 or float32."),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(cosDtype_ != sinDtype,
        OPS_LOG_E(context_->GetNodeName(), "cos and sin dtype must be same."),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(!(cosDtype_ == ge::DT_FLOAT16 || cosDtype_ == ge::DT_BF16 || cosDtype_ == ge::DT_FLOAT),
        OPS_LOG_E(context_->GetNodeName(), "cos/sin dtype only support float16, bfloat16 or float32."),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(yDtype != xDtype_,
        OPS_LOG_E(context_->GetNodeName(), "y dtype must be same as x dtype."),
        return ge::GRAPH_FAILED);
    bool isSamePrecision = xDtype_ == cosDtype_;
    isMixedPrecision_ = (xDtype_ == ge::DT_FLOAT16 || xDtype_ == ge::DT_BF16) && cosDtype_ == ge::DT_FLOAT;
    OPS_ERR_IF(!isSamePrecision && !isMixedPrecision_,
    ...
    gert::Shape yShape = yOutput->GetStorageShape();
    ...
    OPS_ERR_IF(dimNum != CONST_4,
        ...
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(yShape.GetDimNum() != CONST_4,
        OPS_LOG_E(context_->GetNodeName(), "yOutput dim:%ld, should be 4.", yShape.GetDimNum()),
        return ge::GRAPH_FAILED);
    for (int64_t i = 0 ; i < CONST_4; i++) {
        OPS_ERR_IF(xShape.GetDim(i) != yShape.GetDim(i),
            OPS_LOG_E(context_->GetNodeName(), "x and y shape must be same."),
            return ge::GRAPH_FAILED);
    }
    ...
    OPS_ERR_IF(attrs == nullptr,
        ...
        return ge::GRAPH_FAILED);
    auto modeAttr = attrs->GetAttrPointer<int64_t>(0);
    OPS_ERR_IF(modeAttr == nullptr,
        OPS_LOG_E(context_->GetNodeName(), "mode attr is nullptr"),
        return ge::GRAPH_FAILED);
    int64_t mode = *modeAttr;
    OPS_ERR_IF(mode != 1,
    ...
    OPS_ERR_IF(sliceListAttr == nullptr || sliceListAttr->GetSize() < 2,
        OPS_LOG_E(context_->GetNodeName(), "partial_slice must contain start and end."),
        return ge::GRAPH_FAILED);
    ...
    xdim3_ = headDim_;
    OPS_ERR_IF(start_ < 0 || end_ > allHeadDim_ || headDim_ <= 0,
                                                                 ...
                                                                 int64_t ubSize)
{
    ...
    OPS_ERR_IF(bufferSize < totalHeadNum1Size,
        OPS_LOG_E(context_->GetNodeName(), "The D dimension of the input shape is too large for mixed precision."),
        return ge::GRAPH_FAILED);
    ubCalcNNum = GetDiv(bufferSize, totalHeadNum1Size);
    ubCalcNLoop = GetCeilInt(numHeads, ubCalcNNum);
    ubCalcNTail = GetDivRem(numHeads, ubCalcNNum) != 0 ? numHeads - (ubCalcNLoop - 1) * ubCalcNNum : 0;
                                                                 ...
                                                                 int64_t headDimAlign, int64_t ubSize)
{
    ...
    if (ubSize < gatherOffsetSize || bufferSize < totalBatch1Size) {
        OPS_ERR_IF(TilingSplitNMixed(numHeads, headDimAlign, ubSize) != ge::GRAPH_SUCCESS,
                    OPS_LOG_E(context_->GetNodeName(), "TilingSplitNMixed fail."), return ge::GRAPH_FAILED);
        return ge::GRAPH_SUCCESS;
    }
...
ge::graphStatus InplacePartialRotaryMulTiling::TilingSplitSMixed()
{
    ...
    if (gatherOffsetSize >= ubSize_) {
        OPS_ERR_IF(TilingSplitNMixed(numHeads, headDimAlign, ubSize_) != ge::GRAPH_SUCCESS,
                    OPS_LOG_E(context_->GetNodeName(), "TilingSplitNMixed fail."), return ge::GRAPH_FAILED);
        return ge::GRAPH_SUCCESS;
    }
    if (ubSize_ < gatherOffsetSize || bufferSize < totalSeq1Size) {
        OPS_ERR_IF(TilingSplitBMixed(batchSize, numHeads, headDimAlign, ubSize_) != ge::GRAPH_SUCCESS,
                    OPS_LOG_E(context_->GetNodeName(), "TilingSplitBMixed fail."), return ge::GRAPH_FAILED);
        return ge::GRAPH_SUCCESS;
    }
...
ge::graphStatus InplacePartialRotaryMulTiling::TilingSplitMixed()
{
    ...
    OPS_ERR_IF(InitSplitTilingData(batchSizeOut, seqLenOut, numHeadsOut) != ge::GRAPH_SUCCESS,
                OPS_LOG_E(context_->GetNodeName(), "InitSplitTilingData fail."), return ge::GRAPH_FAILED);
    OPS_ERR_IF(TilingSplitSMixed() != ge::GRAPH_SUCCESS,
                OPS_LOG_E(context_->GetNodeName(), "TilingSplitSMixed fail."), return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus InplacePartialRotaryMulTiling::DoTilingMixedPrecision()
{
    ...
    tilingData_.set_allHeadDim(allHeadDim_);
    tilingData_.set_start(start_);
    OPS_ERR_IF(TilingSplitMixed() != ge::GRAPH_SUCCESS,
            OPS_LOG_E(context_->GetNodeName(), "TilingSplitMixed fail."), return ge::GRAPH_FAILED);
    OPS_LOG_I(context_->GetNodeName(), "[mixed tilingKey]: %ld", tilingKey_);
