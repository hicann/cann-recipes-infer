...
#include "../op_kernel/moe_init_routing_v3_arch35_tiling_def.h"
#include "error/ops_error.h"
#include "tiling/tiling_api.h"
#include "moe_init_routing_group_quant_tiling.h"
...
const static int64_t FP8_QUANT_GROUP_SIZE = 128LL;
...
const static int64_t QUANT_MODE_GROUP_FP8_E5M2 = 4LL;
const static int64_t QUANT_MODE_GROUP_FP8_E4M3 = 5LL;
...
class MoeInitRoutingV3Arch35Tiling {
public:
    explicit MoeInitRoutingV3Arch35Tiling(gert::TilingContext *context) : context_(context)
    {}
    ~MoeInitRoutingV3Arch35Tiling()
    {}
...
private:
    ...
    {
        OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::GetTensorShapeDtype(...)");
        ...
        OPS_LOG_E_IF_NULL(context_, shapePtr, ge::GRAPH_FAILED);
        shape = shapePtr->GetStorageShape();
        OPS_LOG_E_IF_NULL(context_, descPtr, ge::GRAPH_FAILED);
    ...
    {
        OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::GetOptionalInputShapeDtype(...)");
    ...
    {
        OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::GetInputAttr(...)");
        const auto *attrPtr = attrsPtr->GetAttrPointer<ATTR_T>(index);
        OPS_LOG_E_IF_NULL(context_, attrPtr, ge::GRAPH_FAILED);
        attr = *attrPtr;
        return ge::GRAPH_SUCCESS;
    }
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::GetPlatformInfo()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::GetPlatformInfo()");
    ...
    OPS_ERR_IF(
        aivCoreNum_ <= 0, OPS_LOG_E(context_->GetNodeName(), "coreNum must be greater than 0."),
        return ge::GRAPH_FAILED);

    uint64_t ubSizePlatForm;
    ascendcPlatform.GetCoreMemSize(platform_ascendc::CoreMemType::UB, ubSizePlatForm);
    totalUbSize_ = static_cast<int64_t>(ubSizePlatForm);
    OPS_ERR_IF(
        totalUbSize_ <= 0, OPS_LOG_E(context_->GetNodeName(), "ubSize must be greater than 0."),
        return ge::GRAPH_FAILED);

    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MoeInitRoutingV3Arch35Tiling::CheckSetPlatformInfo()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CheckSetPlatformInfo()");

    // check aivCoreNum
    OPS_ERR_IF(aivCoreNum_ <= 0, OPS_LOG_E(context_, "Failed to get valid aivCoreNum, current is %ld.", aivCoreNum_),
                return ge::GRAPH_FAILED);
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::DoGetShapeAttrsInfo()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::GetInputAttrsInfo()");
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::DoOpTiling()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::DoOpTiling()");
...
uint64_t MoeInitRoutingV3Arch35Tiling::GetTilingKey()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::GetTilingKey()");
    ...
    } else if (quantMode_ == QUANT_MODE_GROUP_FP8_E4M3 || quantMode_ == QUANT_MODE_GROUP_FP8_E5M2) {
        quantModeFactor = QUANT_MODE_GROUP_FP8_E5M2 - QUANT_MODE_UNQUANT;
    }
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::GetWorkspaceSize()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::GetWorkspaceSize()");
    // 计算workspace大小
    workspaceSize_ = 0;
    ...
    OPS_LOG_D(context_, "Computed workspace size to allocate is %u bytes", workspaceSize_);
    // 设置workspace
    auto *workspacePtr = context_->GetWorkspaceSizes(1);
    OPS_LOG_E_IF_NULL(context_, workspacePtr, ge::GRAPH_FAILED);
    workspacePtr[0] = workspaceSize_;
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MoeInitRoutingV3Arch35Tiling::PostTiling()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::PostTiling()");
    ...
    OPS_ERR_IF(ret != ge::GRAPH_SUCCESS,
                OPS_LOG_E(context_, "Failed to set local memory sizeof %ld bytes", availUbSize_),
                return ge::GRAPH_FAILED);
    // 涉及核间同步的算子必须设置schedule_mode为1，独占全核
    ret = context_->SetScheduleMode(1);
    OPS_ERR_IF(ret != ge::GRAPH_SUCCESS,
                OPS_LOG_E(context_, "Failed to set schedule mode to 1 for kernel that needs sync cores."),
                return ge::GRAPH_FAILED);

    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MoeInitRoutingV3Arch35Tiling::GetInputTensorsInfo()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::GetInputTensorsInfo()");
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::GetOutputTensorsInfo()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::GetOutputTensorsInfo()");
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::GetInputAttrsInfo()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::GetInputAttrsInfo()");

    auto attrsPtr = context_->GetAttrs();
    OPS_LOG_E_IF_NULL(context_, attrsPtr, ge::GRAPH_FAILED);

    MIRV3_CHECK_GE_RET(GetInputAttr<int64_t>(activeNum_, attrsPtr, ATTR_ACTIVE_NUM_INDEX));
    OPS_LOG_D(context_, "Get input attr activeNum = %ld.", activeNum_);
    MIRV3_CHECK_GE_RET(GetInputAttr<int64_t>(expertCapacity_, attrsPtr, ATTR_EXPERT_CAPACITY_INDEX));
    OPS_LOG_D(context_, "Get input attr expertCapacity = %ld.", expertCapacity_);
    MIRV3_CHECK_GE_RET(GetInputAttr<int64_t>(expertNum_, attrsPtr, ATTR_EXPERT_NUM_INDEX));
    OPS_LOG_D(context_, "Get input attr expertNum = %ld.", expertNum_);
    MIRV3_CHECK_GE_RET(GetInputAttr<int64_t>(dropPadMode_, attrsPtr, ATTR_DROP_PAD_MODE_INDEX));
    OPS_LOG_D(context_, "Get input attr dropPadMode = %ld.", dropPadMode_);
    MIRV3_CHECK_GE_RET(GetInputAttr<int64_t>(expertTokensNumType_, attrsPtr, ATTR_EXPERT_TOKEN_NUM_TYPE_INDEX));
    OPS_LOG_D(context_, "Get input attr expertTokensNumType = %ld.", expertTokensNumType_);
    MIRV3_CHECK_GE_RET(GetInputAttr<bool>(expertTokensNumFlag_, attrsPtr, ATTR_EXPERT_TOKEN_NUM_FLAG_INDEX));
    OPS_LOG_D(context_, "Get input attr expertTokensNumFlag = %ld.", expertTokensNumFlag_);
    MIRV3_CHECK_GE_RET(GetInputAttr<int64_t>(quantMode_, attrsPtr, ATTR_QUANT_MODE_INDEX));
    OPS_LOG_D(context_, "Get input attr quantMode = %ld.", quantMode_);
    MIRV3_CHECK_GE_RET(GetInputAttr<int64_t>(rowIdxType_, attrsPtr, ATTR_ROW_IDX_TYPE_INDEX));
    OPS_LOG_D(context_, "Get input attr rowIdxType = %ld.", rowIdxType_);
    // expertStart, expertEnd
    quantBlockSize_ = MX_QUANT_BLOCK_SIZE;
    if (quantMode_ == QUANT_MODE_GROUP_FP8_E4M3 || quantMode_ == QUANT_MODE_GROUP_FP8_E5M2) {
        quantBlockSize_ = FP8_QUANT_GROUP_SIZE;
    }
    const auto *aerPtr = attrsPtr->GetAttrPointer<gert::ContinuousVector>(ATTR_EXPERT_RANGE_INDEX);
    OPS_LOG_E_IF_NULL(context_, aerPtr, ge::GRAPH_FAILED);
    int64_t aerLen = aerPtr->GetSize();
    OPS_ERR_IF(aerLen != 2,
                OPS_LOG_E(context_, "The list length of active_expert_range should be 2, current is %ld.", aerLen),
                return ge::GRAPH_FAILED);
    const int64_t *aerList = reinterpret_cast<const int64_t *>(aerPtr->GetData());
    expertStart_ = aerList[0];
    expertEnd_ = aerList[1];
    OPS_LOG_D(context_, "Extracted input attrs expertStart = %ld, expertEnd = %ld.", expertStart_, expertEnd_);

    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MoeInitRoutingV3Arch35Tiling::CheckSetAttrs()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CheckSetAttrs()");
    ...
    OPS_ERR_IF((expertTokensNumType_ != EXPERT_TOKENS_TYPE_COUNT) &&
                    (expertTokensNumType_ != EXPERT_TOKENS_TYPE_KEY_VALUE),
                OPS_LOG_E(context_, "Attr expert_tokens_num_type currently supports: %ld or %ld, but got %ld.",
    ...
    OPS_ERR_IF(
        ...
        OPS_LOG_E(context_, "Attr expert_num should be in range [1, %ld], current is %ld.", maxExpertNum, expertNum_),
        return ge::GRAPH_FAILED);
    tilingDataPtr_->expertNum = expertNum_;
    // drop_pad_mode 暂不使用，只校验
    OPS_ERR_IF(dropPadMode_ != DROP_PAD_MODE_DROPLESS,
                OPS_LOG_E(context_, "Attr drop_pad_mode currently supports %ld, but got %ld.", DROP_PAD_MODE_DROPLESS,
                        dropPadMode_),
                return ge::GRAPH_FAILED);
    // expertTokensNumFlag 暂不使用，只校验
    OPS_ERR_IF(expertTokensNumFlag_ != true,
                OPS_LOG_E(context_, "Attr expert_tokens_num_flag currently supports True, but got %s",
                        (expertTokensNumFlag_ ? "True" : "False")),
                return ge::GRAPH_FAILED);
    // quantMode
    OPS_ERR_IF(quantMode_ != QUANT_MODE_UNQUANT && quantMode_ != QUANT_MODE_DYNAMIC &&
                    quantMode_ != QUANT_MODE_MXFP8_E5M2 && quantMode_ != QUANT_MODE_MXFP8_E4M3FN &&
                    quantMode_ != QUANT_MODE_GROUP_FP8_E4M3 && quantMode_ != QUANT_MODE_GROUP_FP8_E5M2,
                OPS_LOG_E(context_, "Attr quant_mode currently supports (%ld, %ld, %ld, %ld, %ld, %ld), but got %ld",
                        QUANT_MODE_UNQUANT, QUANT_MODE_DYNAMIC, QUANT_MODE_MXFP8_E5M2, QUANT_MODE_MXFP8_E4M3FN,
                        QUANT_MODE_GROUP_FP8_E4M3, QUANT_MODE_GROUP_FP8_E5M2, quantMode_),
                return ge::GRAPH_FAILED);
    tilingDataPtr_->quantMode = quantMode_;
    // rowIdxType
    OPS_ERR_IF(rowIdxType_ != ROW_IDX_SCATTER && rowIdxType_ != ROW_IDX_GATHER,
                OPS_LOG_E(context_, "row_idx_type currently supports %ld or %ld, but got %ld.", ROW_IDX_SCATTER,
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::CheckSetListAttrs()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CheckSetListAttrs()");

    // expertStart, expertEnd
    OPS_ERR_IF(expertStart_ < 0,
                OPS_LOG_E(context_, "Extracted attr expert_start should be equal or greater than 0, current is %ld.",
                        expertStart_),
                return ge::GRAPH_FAILED);
    OPS_ERR_IF(expertStart_ >= expertEnd_,
                OPS_LOG_E(context_,
                        "Extracted attr expert_start should be less than expert_end, current [expert_start, "
                        "expert_end) is [%ld, %ld).",
                        expertStart_, expertEnd_),
                return ge::GRAPH_FAILED);
    OPS_ERR_IF(expertEnd_ > expertNum_,
                OPS_LOG_E(context_,
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::CheckInputX()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CheckInputX()");

    // rank
    auto rank = static_cast<int64_t>(xShape_.GetDimNum());
    OPS_ERR_IF(rank != 2, OPS_LOG_E(context_, "The rank of input x should be 2, current is %ld.", rank),
                return ge::GRAPH_FAILED);
    // dtype
    using ge::DataType;
    ...
    static const std::unordered_set<DataType> MXQUANT_SUPPORTED_DTYPES = {ge::DataType::DT_FLOAT16,
                                                                          ge::DataType::DT_BF16};
    static const std::unordered_set<DataType> GROUP_QUANT_SUPPORTED_DTYPES = {DataType::DT_FLOAT,
                                                                     ge::DataType::DT_FLOAT16, ge::DataType::DT_BF16};
    unordered_set<DataType> supportedDtypes;
    if (quantMode_ == QUANT_MODE_MXFP8_E5M2 || quantMode_ == QUANT_MODE_MXFP8_E4M3FN) {
        supportedDtypes = MXQUANT_SUPPORTED_DTYPES;
    } else if (quantMode_ == QUANT_MODE_GROUP_FP8_E4M3 || quantMode_ == QUANT_MODE_GROUP_FP8_E5M2) {
        supportedDtypes = GROUP_QUANT_SUPPORTED_DTYPES;
    } else {
    ...
    OPS_ERR_IF(supportedDtypes.count(xDtype_) == 0,
                OPS_LOG_E(context_, "Unsupported dtype of input x: %d under quant_mode: %ld.", xDtype_, quantMode_),
                return ge::GRAPH_FAILED);

    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MoeInitRoutingV3Arch35Tiling::CheckInputExpertIdx()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CheckInputExpertIdx()");

    auto rank = static_cast<int64_t>(expertIdxShape_.GetDimNum());
    OPS_ERR_IF(rank != 2, OPS_LOG_E(context_, "The rank of input expert_idx should be 2, current is %ld.", rank),
                return ge::GRAPH_FAILED);
    int64_t expertIdxDim0 = expertIdxShape_.GetDim(0);
    int64_t xDim0 = xShape_.GetDim(0);
    OPS_ERR_IF(expertIdxDim0 != xDim0,
                OPS_LOG_E(context_, "Unsupported dim0 of input expert_idx: %ld, should be equal to dim0 of x: %ld",
                        expertIdxDim0, xDim0),
                return ge::GRAPH_FAILED);
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::CheckInputScale()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CheckInputScale()");
    ...
    if (expectedRankScale != -1) {
        ...
        OPS_ERR_IF(rankScale != expectedRankScale,
                    OPS_LOG_E(context_, "The rank of input scale should be %ld under quant_mode %ld, current is %ld",
                            expectedRankScale, quantMode_, rankScale),
                    return ge::GRAPH_FAILED);
    }
    if (expectedDim0 != -1) {
        auto dim0 = scaleShape_.GetDim(0);
        OPS_ERR_IF(dim0 != expectedDim0,
                    OPS_LOG_E(context_, "The dim0 of input scale should be %ld under quant_mode %ld, current is %ld",
                            expectedDim0, quantMode_, dim0),
                    return ge::GRAPH_FAILED);
    }
    if (expectedDim1 != -1) {
        auto dim1 = scaleShape_.GetDim(1);
        OPS_ERR_IF(dim1 != expectedDim1,
                    OPS_LOG_E(context_, "The dim1 of input scale should be %ld under quant_mode %ld, current is %ld",
                            expectedDim1, quantMode_, dim1),
                    return ge::GRAPH_FAILED);
    }
    OPS_ERR_IF(scaleDtype_ != ge::DataType::DT_FLOAT,
                OPS_LOG_E(context_, "Unsupported dtype of input scale: %d, should be: DT_FLOAT(%d).", xDtype_,
                        ge::DataType::DT_FLOAT),
                return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MoeInitRoutingV3Arch35Tiling::CheckSetInputs()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CheckSetInputs()");
    ...
    if (activeNum_ != ACTIVE_NUM_MIN_VALUE) {
        ...
        OPS_ERR_IF(
            ...
            OPS_LOG_E(context_, "Attr active_num should equal to bs*k(%ld), current is %ld.", totalLength_, activeNum_),
            return ge::GRAPH_FAILED);
    }
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::CheckOutputExpandedX()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CheckOutputExpandedX()");

    auto rank = static_cast<int64_t>(expandedXShape_.GetDimNum());
    OPS_ERR_IF(rank != RANK_TWO, OPS_LOG_E(context_, "The rank of output expanded_x should be 2, current is %ld.", rank),
                return ge::GRAPH_FAILED);
    int64_t dim0 = expandedXShape_.GetDim(0);
    OPS_ERR_IF(dim0 != totalLength_,
                OPS_LOG_E(context_, "The dim0 of output expanded_x should be %ld, current is %ld.", totalLength_, dim0),
                return ge::GRAPH_FAILED);
    int64_t dim1 = expandedXShape_.GetDim(1);
    OPS_ERR_IF(dim1 != cols_,
                OPS_LOG_E(context_, "The dim1 of output expanded_x should be %ld, current is %ld.", cols_, dim1),
                return ge::GRAPH_FAILED);

    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MoeInitRoutingV3Arch35Tiling::CheckOutputExpandedRowIdx()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CheckOutputExpandedRowIdx()");

    auto rank = static_cast<int64_t>(expandedRowIdxShape_.GetDimNum());
    OPS_ERR_IF(rank != RANK_ONE,
                OPS_LOG_E(context_, "The rank of output expanded_row_idx should be 1, current is %ld.", rank),
                return ge::GRAPH_FAILED);
    int64_t dim0 = expandedRowIdxShape_.GetDim(0);
    OPS_ERR_IF(
        dim0 != totalLength_,
        OPS_LOG_E(context_, "The dim0 of output expanded_row_idx should be %ld, current is %ld.", totalLength_, dim0),
        return ge::GRAPH_FAILED);

    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MoeInitRoutingV3Arch35Tiling::CheckOutputExpertTokensCountOrCumsum()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CheckOutputExpertTokensCountOrCumsum()");
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::CheckOutputExpandedScale()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CheckOutputExpandedScale()");
    ...
    if ((quantMode_ == QUANT_MODE_UNQUANT && isInputScale_ == 1) || (quantMode_ == QUANT_MODE_DYNAMIC)) {
        ...
        expectedDim0 = totalLength_;
        expectedDim1 = CeilAlign<int64_t>(CeilDiv<int64_t>(cols_, quantBlockSize_), 2LL) / 2LL;
        expectedDim2 = 2LL;
    } else if ((quantMode_ == QUANT_MODE_GROUP_FP8_E4M3) || (quantMode_ == QUANT_MODE_GROUP_FP8_E5M2)) {
        expectedRank = RANK_TWO;
        expectedDim0 = totalLength_;
        expectedDim1 = CeilDiv<int64_t>(cols_, quantBlockSize_);
    }
    auto rank = static_cast<int64_t>(expandedScaleShape_.GetDimNum());
    if (expectedRank != -1) {
        OPS_ERR_IF(rank != expectedRank,
                    OPS_LOG_E(context_,
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::CheckOutputs()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CheckOutputs()");
...
void MoeInitRoutingV3Arch35Tiling::LogBaseTilingData()
{
    std::stringstream ss;
    ss << "\n[TilingKey]\n" << tilingKey_ << "\n[WorkspaceSize]\n" << workspaceSize_ << "\n";
    ...
    OPS_LOG_I(context_, "%s", ss.str().c_str());
}

void MoeInitRoutingV3Arch35Tiling::LogVbsTilingData()
{
    std::stringstream ss;
...
void MoeInitRoutingV3Arch35Tiling::LogVmsMiddleTilingData()
{
    std::stringstream ss;
...
void MoeInitRoutingV3Arch35Tiling::LogSortOutTilingData()
{
    std::stringstream ss;
...
void MoeInitRoutingV3Arch35Tiling::LogExpertTokensCountTilingData()
{
    std::stringstream ss;
...
void MoeInitRoutingV3Arch35Tiling::LogGatherOutTilingData()
{
    std::stringstream ss;
...
void MoeInitRoutingV3Arch35Tiling::Tiling4VBSOneCoreCompute(MoeV3Arch35VBSComputeTilingData *vbsTiling)
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::Tiling4VBSOneCoreCompute(...)");
...
void MoeInitRoutingV3Arch35Tiling::Tiling4VBSMultiCoreCompute(MoeV3Arch35VBSComputeTilingData *vbsTiling)
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::Tiling4VBSMultiCoreCompute(...)");

    int64_t needCoreNum = CeilDiv(totalLength_, sortLoopMaxElement_); // 向上取整
    ...
    OPS_ERR_IF(needCoreNum == 0, OPS_LOG_E(opName, "Variale needCoreNum cannot be 0."), return;);
    ...
    do {
        ...
        vbsTiling->perCoreLoops =
            CeilDiv(vbsTiling->perCoreElements, sortLoopMaxElement_); // 每个核处理的loop数
        ...
        int64_t lastCorePerLoopElements =
            CeilDiv(CeilDiv(vbsTiling->lastCoreElements, vbsTiling->lastCoreLoops),
    ...
    OPS_ERR_IF(vbsTiling->lastCoreLastLoopElements <= 0, OPS_LOG_E(opName, "vbs tiling failed"), ;);
}

void MoeInitRoutingV3Arch35Tiling::Tiling4VBSCompute()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::Tiling4VBSCompute()");
...
void MoeInitRoutingV3Arch35Tiling::Tiling4VMSMiddleCompute()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::Tiling4VMSMiddleCompute()");
    ...
    int64_t needCoreNum = CeilDiv(vbsTiling->needCoreNum, MRG_LIST_NUM);
    vmsMiddleTiling->needCoreNum = needCoreNum; // 需要的核数

    LogVmsMiddleTilingData();
}

void MoeInitRoutingV3Arch35Tiling::Tiling4SortOutCompute()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::Tiling4SortOutCompute()");
...
void MoeInitRoutingV3Arch35Tiling::Tiling4ExpertTokensCountCompute()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::Tiling4ExpertTokensCountCompute()");
    ...
    int64_t perCoreElements = CeilDiv(totalElements, aivCoreNum_);
    ...
    int64_t maxElementsPerLoop =
        (availUbSize_ -
         CeilAlign(expertNumElement, UB_BLOCK_SIZE) *
...
void MoeInitRoutingV3Arch35Tiling::Tiling4GatherOutCompute()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::Tiling4GatherOutCompute()");
...
int64_t MoeInitRoutingV3Arch35Tiling::CalcMaxRowIdxPerLoopMxQuant(int64_t perLoopCols)
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::CalcMaxRowIdxPerLoopMxQuant(...)");
    ...
    int64_t scaleSize = 2 * AlignBytes(perLoopCols / quantBlockSize_, inputXDtypeSize_) +
...
void MoeInitRoutingV3Arch35Tiling::Tiling4GatherOutFp8GroupQuant()
{
    OPS_LOG_D(context_, "Entered MoeInitRoutingV3Arch35Tiling::Tiling4GatherOutMxQuant()");
    ...
    int64_t perLoopMaxIndicesElements = CalcMaxRowIdxPerLoopMxQuant(perLoopCols);
    while (perLoopMaxIndicesElements <= 0) {
        perLoopCols = CeilAlign(perLoopCols, quantBlockSize_);
        perLoopMaxIndicesElements = CalcMaxRowIdxPerLoopMxQuant(perLoopCols);
    }
    int64_t colsLoops = CeilDiv(tilingDataPtr_->cols, perLoopCols);
    int64_t lastLoopCols = tilingDataPtr_->cols - (colsLoops - 1) * perLoopCols;
...
ge::graphStatus MoeInitRoutingV3Arch35Tiling::RunTiling()
{
...
ge::graphStatus Tiling4MoeInitRoutingV3(gert::TilingContext* context)
{
    OPS_LOG_I(context->GetNodeName(), "MoeInitRoutingV3Arch35Tiling running.");
    MoeInitRoutingV3Arch35Tiling tiling(context);
    return tiling.RunTiling();
}

ge::graphStatus TilingPrepare4MoeInitRoutingV3(gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(MoeInitRoutingGroupQuant)
    .Tiling(Tiling4MoeInitRoutingV3)
    .TilingParse<MoeInitRoutingV3CompileInfo>(TilingPrepare4MoeInitRoutingV3);
}  // namespace optiling