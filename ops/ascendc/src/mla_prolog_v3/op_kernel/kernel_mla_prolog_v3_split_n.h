...
#ifndef KERNEL_MLA_PROLOG_V3_SPLIT_N_H
#define KERNEL_MLA_PROLOG_V3_SPLIT_N_H
...
class MlaPrologV3VecS1CubS2 {
public:
    __aicore__ inline MlaPrologV3VecS1CubS2(TPipe* pipe, const MlaPrologV3TilingData* __restrict tilingData,
...
private:
    ...
    __aicore__ inline void GetBatchTokenIndex(int64_t &batchTokenIndex, int64_t &batchIndex, int64_t &batchSeqSize, int64_t &batchIndexOffset, int64_t tokenIndex);
    __aicore__ inline void GetCacheOffset(GlobalTensor<int64_t> indexGm, int64_t batchIndex, int64_t batchTokenIndex,
                                          int64_t batchSeqSize, int64_t batchIndexOffset, int64_t rows, int64_t headSize, int64_t &rowsInCurBatch, int64_t &cacheOffset, int64_t &nextBatchOffset);
    ...
    __aicore__ inline void MatmulQcQrSyncDequant(int64_t rmsNormCqResOffset, int64_t weightUqQrOffset,int64_t qcQrResOffset);
    ...
    __aicore__ inline void RmsNormCq(int64_t tokenIndex, int64_t rmsNormCqOffset, int64_t rmsNormCqResOffset, int64_t curVecToken, int64_t curBlockTokenOffset);
...
private:
    ...
    GlobalTensor<int64_t> actualSeqLenGm_;
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::Init(__gm__ uint8_t *tokenX, __gm__ uint8_t *weightDq,
    ...
    if constexpr (MLAPT::actualSeqMode == ACTUAL_SEQ_MODE::EN_Q_LEN) {
        actualSeqLenGm_.SetGlobalBuffer((__gm__ int64_t *)actualSeqLen);
    }
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::ScaleInit( __gm__ uint8_t *dequantScaleX, __gm__ uint8_t *dequantScaleWDq,
    __gm__ uint8_t *deqScaleQcQrW, __gm__ uint8_t *dequantScaleWDkvkr, __gm__ uint8_t *quantScaleCkv, __gm__ uint8_t *quantScaleCkr,
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::MmParamInit() {
    MmCqParamInit();
    MmCkvKrParamInit();
    MmQcQrParamInit();
    MmQnParamInit();
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::MmCqParamInit() {
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::MmCkvKrParamInit() {
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::MmQcQrParamInit() {
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::MmQnParamInit() {
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::VectorBufferInit() {
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::CubeBufferInit() {
    // cube相关Buffer初始化
 ...
 */
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::WorkspaceInit(__gm__ uint8_t *workspace) {
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::UpdateStepBatchParams(int64_t curStepBatchSize) {
 ...
 */
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::Process() {
    ...
    for (int64_t batchOffset = 0; batchOffset < bsSize; batchOffset += baseParams_->stepBatchSize) {
        if constexpr (MLAPT::actualSeqMode == ACTUAL_SEQ_MODE::EN_Q_LEN){
            if (!actSeqState_.inited) {
                ...
                actSeqState_.curIndexOffset = CeilDivT(actSeqState_.curPrefix, static_cast<int64_t>(baseParams_->blockSize));
            }
        }
...
template<typename MLAPT>
template<bool needQnDynamicQuant>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::AicProcess(AicOffset &aicOffset, int64_t batchOffset, int64_t mmQnLoops) {
...
template<typename MLAPT>
template<bool needQnDynamicQuant>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::AivProcess(AivOffset &aivOffset, int64_t batchOffset, int64_t curStepBatchSize, int64_t numHeadOffset, int64_t mmQnLoops) {
    int64_t tokenIndex = batchOffset + aivOffset.curBlockTokenOffset;
    int64_t rmsNormCqResOffset = tokenIndex * baseParams_->headSizeCq;
    if (batchOffset == 0) {
        // 只需要搬运一次
        CopyGlobalParams();
    }
    // if constexpr (MLAPT::cacheMode == CACHE_MODE::PA_BLK_BSND || MLAPT::cacheMode == CACHE_MODE::PA_BLK_NZ) {
    //     UpdateActualSeq(batchOffset, curStepBatchSize);
    // }
    ...
    RmsNormCq(tokenIndex, aivOffset.rmsNormCqOffset, rmsNormCqResOffset, aivOffset.curVecToken, aivOffset.curBlockTokenOffset);
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::ComputeAicOffset(AicOffset &aicOffset, int64_t numHeadOffset) {
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::ComputeAivOffset(AivOffset &aivOffset, int64_t batchOffset) {
...
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::MatmulSplitN(
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::MatmulAndSyncQcQr(AicOffset &aicOffset) {
    if constexpr (MLAPT::enableGroupComputeOpt) {
        // MatmulQc
        // 复用mmCqResGm_ workspace
        MatmulSplitN<rmsNormCqOutputType, mmQcQrOutputType, false, true>(mmQcQrResGm_[aicOffset.qCResOffset], rmsNormCqResGm_[aicOffset.rmsNormCqResOffset], weightUqQrGm_[aicOffset.weightUqOffset],
                mmQcQrParam_, UsedBlockParams{0, QC_CORE_NUM});
        CrossCoreSetFlag<SYNC_MODE_CUBE_VEC, PIPE_FIX>(FINISH_MM_QC);
        // MatmulQr
        MatmulSplitN<rmsNormCqOutputType, mmQcQrOutputType, false, true, false>(mmQcQrResGm_[aicOffset.qRResOffset], rmsNormCqResGm_[aicOffset.rmsNormCqResOffset], weightUqQrGm_[aicOffset.weightQrOffset],
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::MatmulQcQrSyncDequant(int64_t rmsNormCqResOffset, int64_t weightUqQrOffset, int64_t qcQrResOffset) {
    ...
    if (isAFullLoad) {
      LoadL1A(rmsNormCqResGm_[rmsNormCqResOffset], mmQcQrParam_.m, mmQcQrParam_.k, mmQcQrParam_.k, bufParam_);
    ...
    for (int64_t nL1 = 0; nL1 < nL1loops; nL1++) {
        ...
        if (isAFullLoad) {
            MatmulSplitK<rmsNormCqOutputType, mmQcQrOutputType, true>(mmQcQrResGm_[qcQrResOffset], rmsNormCqResGm_[rmsNormCqResOffset],
                weightUqQrGm_[weightUqQrOffset], mmQcQrParam_, bufParam_,
                nL1 * nL1SplitSize, subNL1SplitSize);
        } else {
            MatmulSplitK<rmsNormCqOutputType, mmQcQrOutputType>(mmQcQrResGm_[qcQrResOffset], rmsNormCqResGm_[rmsNormCqResOffset],
                weightUqQrGm_[weightUqQrOffset], mmQcQrParam_, bufParam_,
                nL1 * nL1SplitSize, subNL1SplitSize);
        }
        if constexpr (MLAPT::enableDequantOpt) {
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::PreloadQnAndSync(AicOffset &aicOffset, int64_t mmQnLoops) {
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::MatmulQnWeightPreload(int64_t weightUkOffset, int64_t subLoopTimes) {
    if constexpr (MLAPT::enableGroupComputeOpt) {
...
template<typename MLAPT>
template<bool needQnDynamicQuant>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::MatmulQnSyncDynamicQuantAndMulQr(int64_t qcOffset, int64_t weightUkOffset,
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::CopyInSinCos(int64_t tokenIndex, int64_t curVecToken, int64_t batchOffset, int64_t curStepBatchSize) {
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::CopyGlobalParams() {
    // dequantScaleWDq
    if constexpr (std::is_same<mmInputType, int8_t>::value) {
 ...
 */
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::RmsNormCq(int64_t tokenIndex, int64_t rmsNormCqOffset, int64_t rmsNormCqResOffset,
                                                             int64_t curVecToken, int64_t curBlockTokenOffset) {
    if (blockIdx_ >= curVectorBlockNum_) {
        return;
    }
    // if (blockIdx_ > 0) {return;}
    ...
    if (unlikely (baseParams_->queryNormFlag)) {
        if constexpr (std::is_same<mmQcQrInputType, int8_t>::value) {
            DataCopyPad(dequantScaleQNormGm_[stepTokenIndex], dequantScaleQcQr, {static_cast<uint16_t>(curVecToken), sizeof(dequantScaleQNormType), 0, 0});
        }
    }
}

template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::GetBatchTokenIndex(int64_t &batchTokenIndex, int64_t &batchIndex, int64_t &batchSeqSize, int64_t &batchIndexOffset, int64_t tokenIndex)
{
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::GetCacheOffset(GlobalTensor<int64_t> indexGm, int64_t batchIndex,
                                                                  int64_t batchTokenIndex, int64_t batchSeqSize, int64_t batchIndexOffset,
                                                                  int64_t rows, int64_t headSize, int64_t &rowsInCurBatch, int64_t &cacheOffset,
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::RmsNormAndScatterCkv(LocalTensor<float>& dequantScaleXLocal, LocalTensor<uint8_t>& shareTmpUb,
    ...
    if constexpr ((MLAPT::cacheMode == CACHE_MODE::PA_NZ) || (MLAPT::cacheMode == CACHE_MODE::PA_BSND)){
        ...
        GetBatchTokenIndex(batchTokenIndex, batchIndex, batchSeqSize, batchIndexOffset, rmsNormAndScatterCkvParams.tokenIndex); // TODO 移到外层循环，减少调用次数
        GetCacheOffset(cacheIndexGm_, batchIndex, batchTokenIndex, batchSeqSize, batchIndexOffset, vectorRow_, baseParams_->headSizeCkv, rowsInCurBatch, cacheOffset, nextBatchOffset); 
        printf("batchTokenIndex:%d, batchIndex: %d, batchSeqSize:%d, rowsInCurBatch: %d, cacheOffset: %d, nextBatchOffset: %d\n", 
                batchTokenIndex, batchIndex, batchSeqSize, rowsInCurBatch, cacheOffset, nextBatchOffset);
        ScatterCacheMultiRows<kvCacheType, (MLAPT::cacheMode == CACHE_MODE::PA_BLK_NZ)>(kvCacheGm_, outputLocal,
            ScatterCacheParams{baseParams_->blockSize, cacheOffset, vectorRow_, baseParams_->headSizeCkv, 
                               baseParams_->seq1Size, rmsNormAndScatterCkvParams.tokenIndex}, rowsInCurBatch, cacheOffset, nextBatchOffset);
    }
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::RopeAndScatterKr(
    ...
    if constexpr ((MLAPT::cacheMode == CACHE_MODE::PA_NZ) || (MLAPT::cacheMode == CACHE_MODE::PA_BSND)){
        ...
        int64_t cacheOffset = 0;
        int64_t nextBatchOffset = 0;
        // GetBatchTokenIndex(batchTokenIndex, batchIndex, batchSeqSize, batchIndexOffset, ropeAndScatterKrParams.tokenIndex); // TODO 移到外层循环，减少调用次数
        // GetCacheOffset(cacheIndexGm_, batchIndex, batchTokenIndex, batchSeqSize, batchIndexOffset, vectorRow_, baseParams_->headSizeCkv, rowsInCurBatch, cacheOffset, nextBatchOffset); 
        // ScatterCacheMultiRows<krCacheType, (MLAPT::cacheMode == CACHE_MODE::PA_BLK_NZ)>(krCacheGm_, outputKrLocal,
        // ScatterCacheParams{baseParams_->blockSize, cacheOffset, vectorRow_, baseParams_->headSizeCkv, 
        //                     baseParams_->seq1Size, ropeAndScatterKrParams.tokenIndex}, rowsInCurBatch, cacheOffset, nextBatchOffset);
    }
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::RmsNormRopeScatterCkvKr(int64_t tokenIndex, 
...
template<typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::RopeQr(int64_t ropeQrOffset, int64_t ropeQrResOffset, int64_t curVecToken,
...
template <typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::RopeQrSplitNGroupCase(int64_t ropeQrOffset, int64_t ropeQrResOffset) {
...
template <typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::RopeQrSplitN(const RopeQrSplitNParams& ropeQrSplitNParams){
...
template <typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::DequantQcQrSplitN(const DequantQcQrSplitNParams& dequantQcQrSplitN){
...
template <typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::DequantAndRopeSplitNSyncMMQcQr(
...
// 用于算力切分dequant切N场景
template <typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::DequantQcSplitNGroupCase(int64_t mmQnPreDequantOffset,
...
template <typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::DequantQc(int64_t mmQnPreDequantOffset,
                                                                      int64_t mmQnPreDequantResOffset,
                                                                      int64_t curVecToken,
                                                                      int64_t curBlockTokenOffset)
{
...
template <typename MLAPT>
__aicore__ inline void MlaPrologV3VecS1CubS2<MLAPT>::DynamicQuantQnAndMulQrSyncMMQn(int64_t batchOffset,
