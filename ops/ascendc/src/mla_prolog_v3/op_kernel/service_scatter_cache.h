...
__aicore__ inline void ScatterCacheMultiRows(GlobalTensor<T>& cacheGm, const LocalTensor<T>& inputLocal,
    ...
    if constexpr (!isNz) {
        ...
        if (rowsInCurBatch != scatterCacheParams.row) {
            DataCopy(cacheGm[nextBatchOffset], inputLocal[copyCnt], (scatterCacheParams.row - rowsInCurBatch)*scatterCacheParams.col);
        }
    } else {
        constexpr uint8_t col0 = ALIGN_BLOCK_SIZE / sizeof(T);
        DataCopyParams copyParams {static_cast<uint16_t>(copyCnt / col0), 1, 0, static_cast<uint16_t>(scatterCacheParams.blockSize - 1)};
        DataCopy(cacheGm[cacheOffset], inputLocal, copyParams);
        if (rowsInCurBatch != scatterCacheParams.row) {
