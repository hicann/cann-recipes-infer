...
/*!
 * \file mla_prolog_v3_rms_norm.h
 * \brief
 */

#ifndef MLA_PROLOG_V3_RMS_NORM_H
#define MLA_PROLOG_V3_RMS_NORM_H
...
__aicore__ inline void RmsNormDynamicQuant(const LocalTensor<int8_t>& outputLocal,
                                           const LocalTensor<float> &outputScales,
    ...
    int64_t cnt = rmsNormParams.row * rmsNormParams.col;
    LocalTensor<C> xFp32Local = shareTmpUb.ReinterpretCast<C>();
    RmsNormNormal<T, GammaType, C, C>(xFp32Local, inputGm, gammaLocal,
                                      dequantScaleWDqLocal, dequantScaleXLocal,
    ...
    pipe_barrier(PIPE_V);
    DynamicQuant(xFp32Local, outputScales, xFp32Local, maxInt8Tensor,
                shareTmpUb[(cnt + rowAlign) * sizeof(C)], rmsNormParams.row, rmsNormParams.col);
    pipe_barrier(PIPE_V);
