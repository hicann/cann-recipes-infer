...
本项目基于NPU主要完成了以下优化特性，具体内容介绍可参见[基于SGLang&A3集群的Qwen3-next模型推理部署优化实践](../../docs/qwen3_next/qwen3_next_infer_optimization.md)。
...
1. 创建SGLang镜像。
   ...
   --privileged quay.io/ascend/sglang:main-cann8.3.rc2-a3 /bin/bash
...
5. 编译安装sgl-kernel-npu
	 ```bash
    # 下载sgl-kernel-npu源码
    git clone https://github.com/sgl-project/sgl-kernel-npu -b 20251206
    # 加载patch
    git am ../cann-recipes-infer/models/qwen3-next/patches/SGL-KERNEL-NPU.patch 
    ```
    请参考[安装文档](https://github.com/sgl-project/sgl-kernel-npu/blob/main/python/sgl_kernel_npu/README.mdl)，编译安装sgl-kernel-npu。
    
6. 安装最新Triton-Ascend

	Triton-Ascend是CANN生态对Triton的支持库，请参考[Triton-Ascend安装指南](https://ascend.github.io/triton-ascend/sources/getting-started/installation.html)安装Triton-Ascend。
...
|-------|--------|
   ...
   |`Prefill_Master_Server_IP`| Prefill主节点ip |
   |`Decode_Master_Server_IP`| Decode主节点ip |
...
3. 在Decode节点运行`infer_prefill.sh`
...
2. MTP+SGLang暂不支持图模式；
3. 暂不支持PD分离下开启MTP；
4. CP仅支持PD分离场景开启，启用时P和D部署策略可不同（P可增加cp_size配置），需cp_size<tp_size，需要PD卡数相同（SGLang框架kv传输逻辑限制），CP不支持与DP同时开启（CP复用了DP通信域）；
5. 请确保chunk prefill size <= 71680（triton算子的规格约束）；
6. GDN切分头数存在限制，tp_size需不大于32。
...
### 基于数据集的精度验证
通过以下命令可以执行镜像内已有的数据集进行验证，结果大于0.9即为精度正常：
```sh
cd python/sglang/test
python3 few_shot_gsm8k.py --data-path "/workspace/test.jsonl.txt" --parallel 16 --num-questions 100 --num-shots 5 --port 30002 --temperature 0
```
