...
| 候选 | 最接近参考 | 差异 | 适配风险 |
|------|----------|------|---------|
| A | Qwen3-MoE 32卡 attn4tp8dp | GQA + MoE + Shared Expert 模式一致；Hy3 有 QK Norm、sigmoid router、大词表 | 低 |
| B | Qwen3-MoE 16卡 1tp16ep | 相似但 attn_tp 不同 | 低 |
| C | Qwen3-MoE 16卡 16tp | 纯 TP 模式改为 EP+TP 混合，Hy3 总参数量更大 | 中 |
...
HY3 Attention 架构从 config 实际值确认（非代码推断）：
...
3. **与参考模型一致**：最接近的仓库参考模型 **qwen3-moe**（同为 GQA + MoE + Shared Expert）使用模式一（BSH 连续缓存），当前 HY3 实现已完全对齐。
...
1. **QK Norm 兼容性**：HY3 的 QK Norm 是 per-head RMSNorm(head_dim)，需要访问每个 head 的独立维度。当前实现的 `view(bsz, q_len, num_heads, head_dim)` → Norm → `view(bsz, q_len, -1)` 链在 BSH 下自然且高效（仅 view 操作，无数据拷贝）。
...
1. **GQA 无特殊需求**：FA v2 的额外能力（learnable_sink、dequant_scale_key/value、query_quant_mode）均用于 MLA/量化/Sink Token 场景，HY3 GQA 不需要。
...
| 功能 | FA v1 参数名 | FA v2 参数名 | HY3 当前使用 |
|------|------------|------------|------------|
...
| 特性 | HY3（当前实现） | qwen3-moe（参考） | gpt-oss（参考） |
|------|---------------|-------------------|----------------|
...
HY3 当前实现与 qwen3-moe 参考高度一致，唯一的架构差异（QK Norm）已正确适配。
...
QK Norm 是 HY3 特有的 per-head RMSNorm，不存在现成的融合算子覆盖此路径。`npu_kv_rmsnorm_rope_cache` 的 RMSNorm 是 layer-level（非 per-head），不适用于 QK Norm。
...
**HY3 当前实现** (`modeling_hy_v3.py:56-61`):
```python
...
**匹配评估**: 完全匹配。`npu_rms_norm(x, gamma, eps)` 计算公式 `x / sqrt(mean(x^2) + eps) * gamma`，与 HY3 RMSNorm 数学等价。qwen3-moe、deepseek-r1、deepseek-v3.2-exp 等所有仓库模型均使用此算子。
...
**HY3 当前实现** (`modeling_hy_v3.py:629-662`):
```python
# 当前：分两步 — 先 norm，再 add
...
**HY3 当前实现** (`modeling_hy_v3.py:340-362`):
```python
...
HY3 的 router 算法与 `npu_moe_gating_top_k(norm_type=1)` 的语义差异：

| 步骤 | HY3 原始行为 | `npu_moe_gating_top_k(norm_type=1, bias=...)` |
|------|------------|----------------------------------------------|
...
**差异的本质**: HY3 的 `e_score_correction_bias` 只影响专家选择（topk 排序），不影响最终的专家权重（权重始终取原始 sigmoid 值）。而 `npu_moe_gating_top_k` 的 bias 同时影响选择和权重。
...
**HY3 当前实现** (Dense FFN `modeling_hy_v3.py:316-322`):
```python
def forward(self, x):
...
- API 仅支持无 bias 的 FFN（HY3 FFN 无 bias，匹配 ✓）
...
**分析**: HY3 的 Dense FFN（layer 0, intermediate=13312）计算量较小（约 0.33B FLOPs），且仅 1 层，可能不满足 `npu_ffn` 的门槛要求。**优先使用 `npu_swiglu` 激活融合（方案 2.4），`npu_ffn` 全融合作为进阶选项**。
...
**匹配评估**: **不适配。** 原因：
- **硬件约束**: API 文档明确标注仅支持 Atlas A3 系列，HY3 现部署在 Atlas A3 上，此 API 变为可用。
...
QK Norm 是 HY3 特有的 per-head RMSNorm(128)。`npu_kv_rmsnorm_rope_cache` 的 RMSNorm 是 layer-level（非 per-head），**不适用于 QK Norm**。
...
| 算子 | 不适配原因 | 硬约束证据 |
|------|----------|----------|
| `npu_kv_rmsnorm_rope_cache` | MLA 专用算子，做 layer-level KV norm + RoPE + cache 写入。HY3 是 GQA + per-head QK norm，完全不匹配 | API 文档明确描述为 MLA 结构专用 |
| `npu_mla_prolog_v3` | MLA prolog 算子，融合 Q/KV 投影 + RMSNorm + RoPE + cache。HY3 为 GQA 架构 | API 文档明确要求 MLA absorb 模式 |
...
| **P1** | MoE Router (79处) | Python sigmoid+topk+gather+norm 多步 | `torch_npu.npu_moe_gating_top_k` | 融合 sigmoid+topk+routing 全流程为单算子 | **精度验证**（bias 语义差异：HY3 bias 在 sigmoid 后，API 在 sigmoid 前） | Router 输出的 topk_index 和 topk_weights 精度对比 |
...
| ⛔ | MoE Combine+Add+RMSNorm | 手动 AllToAll + finalize + shared + add | `npu_moe_distribute_combine_add_rms_norm` | 不适配 | **不可用**: Atlas A3 only，HY3 部署在 Atlas A2 | N/A |
...
| MoE Routing | init_routing_v2 + re_routing + finalize | init_routing_v2 + MC2 dispatch/combine (Decode) 或 all_to_all (Prefill) | HY3 统一使用 all_to_all，qwen3-moe Decode 用 MC2 |
| QKV/O Proj | ParallelLinear | ParallelLinear | **已对齐** |

**关键发现**: HY3 与 qwen3-moe 在 Attention 和并行 Linear 层面已对齐，主要差距在 Norm 和 MoE Router 的融合算子使用上。qwen3-moe 自身也未使用 `npu_swiglu`，说明此优化在仓库中尚无 GQA MoE 模型的验证先例，HY3 可作为首个试点。
...
- [完成] `npu_rms_norm` 替换 HYV3RMSNorm.forward — modeling_hy_v3.py:56-78
  ...
  - dtype 约束: BF16 input + BF16 weight ✓ (HY3RMSNorm weight 默认 float32, API 文档确认 bfloat16 input + float32 weight 支持 ✓)
...
- [完成] `npu_add_rms_norm` residual+norm 融合 — modeling_hy_v3.py:56-78, 651-686, 774-790
  ...
  - eps=1e-5 适配: `self.variance_epsilon` 从 config.rms_norm_eps 读取 (HY3 config 为 1e-5), 直接传递给 `npu_add_rms_norm` 和 `npu_rms_norm`
...
- ⛔: MoE Combine+Add+RMSNorm → `npu_moe_distribute_combine_add_rms_norm` — Atlas A3 only, HY3 部署在 A2
...
- [分析] `npu_moe_distribute_combine_add_rms_norm`: **不可行** — API 计算 `norm(combine+shared+residual)` 与 HY3 的 deferred residual 模式 `norm(signal)+residual` 数学不等价，需全局重构 80 层 residual flow
- [分析] `npu_ffn(expert_tokens=..., activation='swiglu')`: **不可行** — API 硬约束：swiglu 模式不接受 expert_tokens 参数（仅 silu 模式支持），且 swiglu 仅支持 float16（HY3 用 bfloat16）
