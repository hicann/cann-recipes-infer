# Deepseek-V3.2 Inference on NPU
## 概述
DeepSeek团队发布了最新的模型DeepSeek-V3.2，在各项指标上都达到了SOTA水平。本样例基于[Deepseek开源代码](TODO)进行迁移，并在CANN平台上完成对应的优化适配，可在华为 Atlas A3 集群上运行起来。

- 本样例的并行策略和性能优化点详细介绍可参见[NPU DeepSeek V3.2推理优化实践](../../docs/models/deepseek-v3.2/deepseek_v3.2_inference_guide.md)。
...
### 下载权重
  ...
  下载[Deepseek-V3.2原始fp8权重](TODO)，并上传到Atlas A3各节点某个固定的路径下，比如`/data/models/DeepSeek-V3.2-fp8`。
...
### 转换权重
  ...
  ```
  # 转换为bf16权重
  cd models/deepseek-v3.2
  python utils/convert_model.py --input_fp8_hf_path /data/models/DeepSeek-V3.2-fp8 --output_hf_path /data/models/DeepSeek-V3.2-bf16
  ```
...
- 在各个节点上修改 config/ 路径下需要执行的yaml文件中的model_path路径。关于YAML文件中的更多配置说明可参见[YAML参数描述](./config/README.md)。
  ```
  model_path: "/data/models/DeepSeek-V3.2-bf16/"
  ```

- 在各个节点上修改 infer.sh 文件中的YAML_FILE_NAME，指定为上一步需要执行的yaml文件名。默认的yaml路径为32卡推理。
  ```
  export YAML_FILE_NAME=deepseek_v3.2_rank_64_64ep_prefill.yaml
  ```
 
  > **Note**: 本样例Prefill支持32-128卡，Decode支持32-128卡，可分别在config下的deepseek_v3.2_rank_64_64ep_prefill.yaml和deepseek_v3.2_rank_128_128ep_decode.yaml文件中修改world_size配置。
...
## 附录
### FAQ
- **HCCL_BUFFSIZE不足问题**：如果报错日志中出现关键字"HCCL_BUFFSIZE is too SMALL, ..., NEEDED_HCCL_BUFFSIZE...= xxx MB, HCCL_BUFFSIZE=200MB, ..."，可通过配置环境变量 `export HCCL_BUFFSIZE=xxx` 配置实际需要的大小解决。HCCL_BUFFSIZE参数介绍可[参考昇腾资料](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/83RC1alpha002/maintenref/envvar/envref_07_0080.html)中的详细描述。