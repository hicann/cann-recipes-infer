...
- **transformers 兼容**: 适配 v5.5.0 (_tied_weights_keys dict 格式, tie_weights **kwargs)
...
- [修复] transformers 5.5.0 兼容性: _tied_weights_keys 从 list 改为 dict 格式; tie_weights() 添加 **kwargs 接受 recompute_mapping 参数 -- models/modeling_gemma4.py:826,880
...
- 精度基准: 首次多卡基线, 无单卡对照 (BF16 权重 51.6GB 超出单卡 32GB HBM)
- 备注: 输出与 implementer 自验证结果完全一致, 表明并行化改造未引入精度退化

### 性能验证 (阶段 1 并行化, 2026-04-15)
- 状态: 首次基线采集 (缺少标准基线, 无前值对比)
- 配置: 8x Ascend 910B4, eager 模式, BS=8, input_len=256, decode_steps=32
...
每卡参数显存 ~6.5 GB + KV cache 6.9 GB = 13.4 GB，32 GB HBM 仍有余量。
...
- 验证时间: 2026-04-15, 配置: 8x 910B4 eager BS=8 input_len=256 decode_steps=32
...
- [发现] /tmp 磁盘空间不足 (overlay 196G 100%), GE kernel 编译失败
- [修复] 设置 TMPDIR=/data2/tmp 使用 /data2 (6.4T 可用) 作为临时目录
...
**分支**：`refactor/gemma4-framework`，起点 commit 是 pre-refactor baseline `7f7d7b5`（阶段 4 末态）。

### 提交序列（自底向上）

```
f9dde41 refactor(gemma-4): rename dir + drop runner/infer/model_setting
5a145b5 refactor(gemma-4): register to framework + YAML schema 4-section
8f5ed69 refactor(gemma-4): migrate modeling to InferenceConfig + CommManager
925b8f4 fix(gemma-4): wire up framework glue (process_weights_after_loading, atten_mask=None decode, drop __init__.py, export PYTHONPATH)
41cd1da fix(gemma-4): MoE row_idx_decode uses batch_size_per_dp_rank for ge_graph
```
...
- `models/gemma4_26b_a4b/FRAMEWORK_MIGRATION.md` — 本次迁移的完整说明
- `.claude/skills/model-infer-framework-migrate/SKILL.md` — 抽象为通用 skill，供其他模型迁移参考
