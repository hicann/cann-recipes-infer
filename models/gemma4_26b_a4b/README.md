...
Gemma-4-26B-A4B是Google于2026年开源的多模态MoE大语言模型，包含视觉塔与语言MoE解码器两部分。本样例基于[transformers](https://github.com/huggingface/transformers/blob/v5.0.0/src/transformers/models/gemma4/modeling_gemma4.py)的实现，仅适配Language MoE Decoder路径（视觉塔暂不适配），并完成对应的NPU优化适配，包括KVCache异构连续缓存管理、融合算子替换、GE图模式与npugraph_ex图模式加速、MoE EP并行（local_experts / MC2 dispatch_v2 双路径自动切换）。
...
- [agentic/optimization_report.md](agentic/optimization_report.md) — 终态技术报告
- [agentic/progress.md](agentic/progress.md) — 各阶段过程归档
...
| 项 | 位置 | 说明 |
|----|------|------|
| 权重 | HuggingFace `google/gemma-4-26B-A4B` | 下载到本地路径 |
| chat_template | 仓内 `chat_template.jinja` | 拷贝到权重目录 |
...
2. 安装Ascend Extension for PyTorch（torch_npu）。
   ...
   请从[软件包下载地址](https://gitcode.com/Ascend/pytorch/releases/v7.3.0-pytorch2.8.0)下载`torch_npu-2.8.0.post2-cp311-cp311-manylinux_2_28_${arch}.whl`安装包，并参考[torch_npu安装文档](https://www.hiascend.com/document/detail/zh/Pytorch/730/configandinstg/instg/docs/zh/installation_guide/installation_via_binary_package.md)进行安装。
...
请从 HuggingFace 下载 Gemma-4-26B-A4B 原始权重到本地路径，例如 `/data/models/gemma-4-26B-A4B`：
...
huggingface-cli download google/gemma-4-26B-A4B --local-dir /data/models/gemma-4-26B-A4B

# 或通过 git lfs（需先 git lfs install）
git clone https://huggingface.co/google/gemma-4-26B-A4B /data/models/gemma-4-26B-A4B
```

下载完成后，将本仓库提供的 `chat_template.jinja` 拷贝到权重目录：

```bash
cp models/gemma4_26b_a4b/chat_template.jinja /data/models/gemma-4-26B-A4B/
```

> 说明：HuggingFace 上的 `tokenizer_config.json` 不带 `chat_template`，但本仓 framework 在 offline 推理路径会调用 `tokenizer.apply_chat_template`；AutoTokenizer 在 transformers 5.0+ 会自动从 model_path 加载侧车 `chat_template.jinja`，因此把这份模板拷到权重目录后无需任何代码修改。

> 注意：模型权重 51.6 GB（BF16），需多卡部署，推荐 8 卡。
...
详见 [optimization_report.md](agentic/optimization_report.md)，主要优化包括：
...
|---|---|---|---|
| A2 | eager 基线 | 312 ms | 98.5 ms |
| A2 | ge_graph + EP8 | 189 ms | 15 ms |
| A3 | ge_graph + EP8 | 76 ms | 10.2 ms |
| A3 | npugraph_ex + EP8 | 102 ms | 11.6 ms |

> 详细性能拆解（含逐阶段贡献、A3 MoE 双路径 mc2 vs local_experts 对比）参见 [optimization_report.md](agentic/optimization_report.md)。
