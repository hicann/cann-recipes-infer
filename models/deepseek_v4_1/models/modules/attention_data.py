...
def build_engram_hash_inputs(cp_size, config, input_ids, is_prefill,
                             actual_seq_lengths_q, batch):
    ...
    ngram_size] (True = the token at that shift is usable). Either
    output is None when not applicable (cp_size != 1 or ngram_size
    <=1 disables engram hashing entirely).
    """
    prefix_input_ids = None
    ngram_shift_mask = None
    prefix_len = max(0, int(config.engram_max_ngram_size) - 1) if cp_size == 1 else 0
    if prefix_len:
        if is_prefill:
            # Packed prefill tokens must not borrow history across a request
            # boundary or an image token, dist is the gap to the nearest blocker.
            ...
            offset = 0
            for seq_len in actual_seq_lengths_q.tolist():
                seq_len = int(seq_len)
                ...
                dist = local_pos - torch.cummax(local_pos.masked_fill(~image_mask, -1), 0).values
                # pad count = max(0, prefix_len + 1 - dist)
...
class AttnMetaData(nn.Module):
    def __init__(self, config, comm_manager, infer_config: InferenceConfig):
        super().__init__()
        self.config = config
        ...
        self.cache_dtype_map = {
            "float8": torch.float8_e4m3fn, "unquant": torch.bfloat16
        }
            ...
            if ratio == 4:
                ...
                if self.li_cache_quant_mode == "float8":
                    ratio_cache["li_key_dequant_scale"] = self.create_cache(
                        cmp_block_num, 1, torch.float32, device)
    ...
    def build_attn_metadata(self, input_ids, position_ids, forward_metadata, batch):
        if self.cp_size != 1:
            raise NotImplementedError("The Window FA reference currently requires cp_size=1.")
        ...
        engram_prefix_ids, engram_shift_mask = build_engram_hash_inputs(
            self.cp_size, self.config, input_ids, is_prefill,
            actual_seq_lengths_q, batch,
        )
        ...
        win_topk_ids = self.generate_win_topk_ids(position_ids)
    ...
    def get_cp_metadata(self, input_ids, is_prefill, attn_metadata, cp_metadata):
