...
class PanguEmbeddedRunner(ModelRunner):
    def __init__(self, runner_settings):
        ...
        self.enable_weight_nz = runner_settings.get("model_config").get("enable_weight_nz", False)
    ...
    def init_model(self):
        ...
        from models.configuration_openpangu_dense import PanguEmbeddedConfig as config
        super().init_model(PanguEmbeddedForCausalLM, config)
    ...
    def graph_compile(self):
        ...
        # disable optimize_noop_ops optimization
    ...
    def model_inference(self, model_inputs, is_prefill=False, warm_up=False):
        ...
        logger.info(
            "%s inference time cost of %s is %.2f ms",
            self.model_name,
            inference_stage,
            inference_time * 1000,
        )
    ...
    def model_generate(self, prompts, warm_up=False):
        ...
        # get init input_dict
        # attenMask shape must be (2048, 2048) / (1, 2048, 2048) / (1, 1, 2048, 2048)
        # when sparse mode = 2, but now it's (4096, 4096).
        # [FUNC:CheckMaskShapeCrossSparse][FILE:prompt_flash_attention_tiling_v2.cpp][LINE:1455]
