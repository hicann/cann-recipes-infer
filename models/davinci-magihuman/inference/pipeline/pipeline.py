...
from inference.common import EvaluationConfig, EngineConfig, ModelConfig, parse_config
...
class MagiPipeline:
    ...
    def __init__(
        ...
        sr_arch_config: ModelConfig = None,
        ...
        if evaluation_config.use_sr_model:
            if sr_arch_config is None or engine_config is None:
                config = parse_config()
                sr_arch_config = config.sr_arch_config
                engine_config = config.engine_config
            engine_config.load = evaluation_config.sr_model_path
            if env_is_true("CPU_OFFLOAD"):
                sr_model = get_dit(sr_arch_config, engine_config, device=torch.device("cpu"))
            else:
                sr_model = get_dit(sr_arch_config, engine_config)
        else:
            sr_model = None
    ...
    def run_offline(
        ...
        if torch.distributed.get_rank() == torch.distributed.get_world_size() - 1:
            ...
            imageio.mimwrite(
                video_path, video_np, fps=self.evaluation_config.fps, quality=8,
