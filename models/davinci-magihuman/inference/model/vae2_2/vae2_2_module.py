...
class ScatterFwdAllGatherBackwardOverlap(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_chunks = [
            torch.zeros(grad_shape, device=grad_output.device, dtype=grad_output.dtype)
...
class Resample(nn.Module):
    ...
    @torch.compile
    def forward(self, x, feat_cache=None, feat_idx=None, group: torch.distributed.ProcessGroup = None):
        if feat_idx is None:
            feat_idx = [0]
        if one_plus_world_size(group):
            ...
            if feat_cache is not None:
                ...
                if feat_cache[idx] is None:
                    ...
                    if cache_x.shape[2] < 2 and feat_cache[idx] is not None and feat_cache[idx] != "Rep":
                        ...
                        prev_frame = feat_cache[idx][:, :, -1, :, :].unsqueeze(2).to(cache_x.device)
                        cache_x = torch.cat([prev_frame, cache_x], dim=2)
                    if cache_x.shape[2] < 2 and feat_cache[idx] is not None and feat_cache[idx] == "Rep":
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # apply attention
        # NPU path: use torch_npu.npu_fused_infer_attention_score
        # Input layout: BSND (batch=bt, seqlen=1, num_heads=-1, head_dim=c)
        bt, s, n, d = q.shape
        scale = 1.0 / (d ** 0.5)
        ...
        # NPU FA has limit on head_dim <= 512
        if d <= 512:
            x, _ = torch_npu.npu_fused_infer_attention_score(
                q, k, v,
                num_heads=n,
                scale=scale,
                input_layout="BSND",
            ...
            # Fallback to regular attention for large head_dim
...
class DupUp3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, first_chunk=False) -> torch.Tensor:
        ...
        x = x.view(
            x.size(0), self.out_channels, self.factor_t, self.factor_s, self.factor_s,
            x.size(2), x.size(3), x.size(4),
        )
        x = x.permute(0, 1, 5, 2, 6, 3, 7, 4).contiguous()
        x = x.view(
            x.size(0), self.out_channels,
            x.size(2) * self.factor_t, x.size(4) * self.factor_s, x.size(6) * self.factor_s,
        )
        if first_chunk:
            x = x[:, :, self.factor_t - 1:, :, :]
        return x
...
class Wan2_2_VAE:
    def __init__(
        ...
        self.vae = (
            _video_vae(
                pretrained_path=vae_pth, z_dim=z_dim, dim=c_dim,
                dim_mult=dim_mult, temperal_downsample=temperal_downsample,
            )
            .eval()
