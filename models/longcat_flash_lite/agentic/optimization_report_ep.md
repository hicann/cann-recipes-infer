...
LongCat-Flash-Lite 在 8 卡 EP 部署下面向中等 batch、吞吐优先场景。下文给出 EP 路径的并行切分、KVCache 与 Attention 改造、算子融合、图模式适配的最终方案，并附 A2 / A3 两代硬件上的实测性能与精度结论。MoE 模块按专家并行（EP）切分到 8 卡，N-gram Embedding 单独按 TP 切分，Dense MLP 与主 Embedding 保持 DP-replicated 以减少同步通信。
...
Prefill 与 Decode 走两条不同的 FA v2 路径，布局、mask 模式与 cache 拉取方式独立选择；batch>1 的 packed prefill 通过 cumulative offsets 喂入 `actual_seq_lengths`。
...
A3 上各阶段的累计优化效果、A2 同硬件演进、不同 batch 与输入长度下的吞吐扩展分别汇总于下，最大收益来自 MC2 dispatch_v2 替换 prefill-only AllToAll 与 GE 图整图编译。

### 8.1 累计优化路径

A3 平台 batch=2 的各阶段实测如下，行间是累加关系，最右一列对照初始 TP=8 eager 基线：
...
最终 A3 Decode 单步从 273 ms 降至 8.10 ms，累计加速 33.7×。

### 8.2 吞吐扩展性

A3 平台固定 EP 拓扑（attention 走 attn_tp=4 + attn_dp=2），扫多组输入长度与 batch 大小验证吞吐扩展性。
...
三组实测共同反映出 EP+DP 拓扑在该模型上的扩展规律：

- batch=8 相比 batch=2 吞吐扩展约 3.0–3.4×（理论上限 4×），中等 batch 下扩展性良好，没有出现 EP 通信瓶颈。
- 输入从 1K 增至 4K，Decode 时延仅 +12%；attention 走 PA 分块，每步只读活跃 KV cache block，O(seq) 增长被通信开销稀释。
- Prefill 在长输入下超线性增长（输入 ×4 时 Prefill 从 129 ms 涨到 1536 ms，约 ×11.9），attention 的 O(seq²) 计算在此 regime 占主导。
