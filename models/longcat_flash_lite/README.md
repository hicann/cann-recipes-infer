...
基于 MLA + Sparse MoE + N-gram Embedding 架构的稀疏专家混合大语言模型。本样例基于 LongCat-Flash-Lite 开源代码进行迁移，并完成对应的 NPU 优化适配，覆盖 Paged Attention 缓存管理、融合算子替换、图模式加速、专家并行路径、W8A8 量化、Decode 多流重叠与多 batch 支持。
...
- **优化点参考**：本样例已落地的主要优化点（详细方案见下方 agentic 报告）：
  - **packed sequence 布局**：MoE、MLA、N-gram Embedding、LM Head 全链路按 token 维展平的 packed sequence 组织，减少各模块之间的布局转换。
  ...
  - **W8A8 量化**：MoE 专家与 Dense MLP 走 W8A8 int8（MLA 保持 BF16），相同配置下 Decode 较 BF16 下降 14% 至 16%、Prefill 下降约 36%。
  - **Decode 多流 MoE 重叠**：Decode 阶段把整段 MoE 调度到副流，与主流的另一子层并行以隐藏其耗时，目标配置下约九成被隐藏、设备侧单步耗时下降约 14%。
...
## 权重准备
可按照实际部署需求，选择下载量化权重或原始权重。
### 量化权重（推荐）
从 [量化权重下载链接](https://cann-ai.obs.cn-north-4.myhuaweicloud.com/cann-quantization/LongCat-Flash-Lite-W8A8.tar) 下载LongCat-Flash-Lite W8A8 量化权重并解压缩到本地路径（例如 `/data/models/LongCat-Flash-Lite-W8A8`），并将 yaml 内 `model_config.model_path` 改为该路径。

### 原始权重
从 HuggingFace 仓库（[meituan-longcat/LongCat-Flash-Lite](https://huggingface.co/meituan-longcat/LongCat-Flash-Lite)）下载 LongCat-Flash-Lite 原始权重到本地路径（例如 `/data/models/LongCat-Flash-Lite`），并将 yaml 内 `model_config.model_path` 改为该路径：
...
1. 配置 yaml：`model_config.model_path` 改为本地权重路径（必改）；其他参数按需调整，通用字段含义见 [YAML 参数描述](../../docs/common/inference_config_guide.md)。
   ...
   | 参数名 | 类型 | 默认值 | 含义 |
   | --- | --- | --- | --- |
   | `ngram_embed_tp_size` | int | `0` | N-gram 子表的独立 TP 切分数，`0` 表示跟随 `embed_tp_size`。 |
   | `enable_moe_stream_overlap` | bool | `false` | Decode 阶段把整段 MoE 调度到副流，与主流另一子层并行（多流重叠）。 |
   | `enable_limit_core_num` | bool | `true` | 多流重叠时限制主流与 MoE 副流各自占用的核数，避免两条流争用。 |

   配置文件清单：
   - `config/longcat_flash_lite_rank_8_8tp.yaml` — 8 卡 TP，BF16，低延迟
   - `config/longcat_flash_lite_rank_8_8ep.yaml` — 8 卡 EP，BF16，平衡延迟与吞吐
   - `config/longcat_flash_lite_rank_8_8ep_w8a8.yaml` — 8 卡 EP，W8A8 + Decode 多流重叠
   - `config/longcat_flash_lite_rank_16_16ep_w8a8.yaml` — 16 卡 EP，W8A8 + Decode 多流重叠（8 卡等比扩展）

   > W8A8 yaml 需把 `model_path` 指向 W8A8 checkpoint（compressed-tensors 量化），多流重叠已在 `custom_params` 内默认开启。
...
2. 准备推理 prompt：
   ...
   > 若长序列场景出现 OOM，按先后顺序尝试：减小 `scheduler_config.batch_size`、增大 `kvp_size`、切换 W8A8 量化权重。
...
3. 执行统一推理脚本。
   ...
   bash executor/scripts/infer.sh --model longcat_flash_lite --yaml longcat_flash_lite_rank_8_8ep_w8a8.yaml
   ```
...
A3 8 卡实测性能。前三行为 BF16，对应 `config/` 下的 TP / EP yaml（高吞吐行在 EP yaml 上将 `batch_size` 改为 8）；末行为 W8A8 量化叠加 Decode 多流重叠的部署配置，4K 输入需按「推理执行」第 2 步替换为对应长度 prompt：
...
| `rank_8_8ep_w8a8.yaml`（EP，量化 + 多流） | W8A8 | 8 | 4096/1024 | 8×A3 | — | 7.12 | 1124 |

> 详细性能拆解（含逐阶段贡献、A2 / A3 跨硬件对比、W8A8 与多流重叠拆解）参见 [optimization_report_tp.md](agentic/optimization_report_tp.md)（TP 路径）与 [optimization_report_ep.md](agentic/optimization_report_ep.md)（EP 路径）。
