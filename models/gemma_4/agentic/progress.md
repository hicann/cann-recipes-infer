...
| FA 算子版本 | FA v1 (npu_fused_infer_attention_score) | 当前已在使用，head_dim=256/512 均在 FA v1 约束范围内（Q_S=1 时 D<=512，Q_S>1 时 D<=512）；滑窗可用 sparse_mode=4+pre_tokens；FA v2 无额外收益 |
...
3. **sliding_window 与 PA 的交互**：sliding 层只需最近 1024 token 的 KV。连续缓存下通过 FA 的 sparse_mode=4 + pre_tokens=1024 即可实现，物理缓存虽分配了全序列长度但实现简单。PA 模式下滑窗需要额外逻辑管理 block 回收/重用，复杂度更高。
...
**Prefill 阶段**：
- sliding 层：使用 sparse_mode=3（因果 mask），FA 自动处理滑窗范围内的注意力（当 seq_len<=1024 时等价于全因果）
...
- Prefill sliding 层: sparse_mode=4, pre_tokens=1024 (滑窗因果 mask)
...
## 阶段 6：TND+PA + 算子融合 + MC2 改造（2026-05-19 完成）

> 平台：Atlas A3，CANN 9.0.0，torch 2.8.0 / torch_npu 2.8.0
> 测试条件：BS=8, input_len=256, max_new_tokens=32

### Layout 决策（attention 改造选型）

| 选项 | sliding (D=256) | full (D=512) | 结论 |
|---|---|---|---|
| 纯 TND-PA | 可（CANN 9.0.0 TND 非 MLA 实测扩到 D=256）| 不可（doc + runtime 拒 D=512 非 MLA）| 不可行 |
| 纯 BNSD-PA | 可 | 可 | 实测 FA AICPU `errorCode=0x91` decode warmup 挂（BSND-PA + GQA + flat-H + block_table 未通 tilingkey），不采用 |
| **hybrid TND + BNSD** | TND + PA + flat-H cache | BNSD + PA + cache `.transpose(1,2)` 非连续视图 | **采用**（过渡形态，待算子白名单后续支持后回归纯 TND）|

若 CANN 后续将 D=512 加入 FA v2 TND 非 MLA 白名单，可回归纯 TND（详见 optimization_report.md §9 算子需求 #1）。

### 实施要点

- 三 yaml 删 `enable_pa: false`，走 framework PA 路径
- `Gemma4Attention.forward` 按 `is_sliding` 分发到 `_attn_tnd_sliding` 或 `_attn_bnsd_full`；sliding 用 `sparse_mode=4 + pre_tokens=sliding_window` band mask，full 用 `sparse_mode=3` prefill / `0` decode
- 顶层 packed 1D `[T, H]` 全程；`cu_q - 1` index_select 取 last token；lm_head `all_gather_into_tensor`
- RoPE：sliding `npu_rotary_mul(rotary_mode='half')`（3D TND → 4D BSND 零拷贝视图后调用，匹配原版 Phase 0 实现）；full `npu_apply_rotary_pos_emb` partial D=128
- MoE EP decode 走 MC2 `dispatch_v2 / combine_v2`，prefill 走双轮 AllToAll（`moe_infer_double_routing`）；gemma-4 EP=8 下每卡 16 个专家未触发 A2 的 MC2 单卡专家上限，因此模型代码内不保留 A2 fallback 分支
- FA op dispatch：prefill `torch.ops.npu`、decode-in-ge_graph `torchair.ops`

### 弃用 / 失败实验

- **Unified BSND-PA**：曾试图把 sliding/full 都统一 BSND-PA + flat-H cache，CANN 9.0.0 该 tilingkey 未走通，FA AICPU 挂，回到 hybrid。
- **Variable-length padded prefill**：full BNSD `view(B, S, N, D)` 要求 T=B*S 等长，曾用 `npu_scatter_nd_update_` + argmax-over-mask padding 支持变长，ge_graph 性能严重下降（dynamic-shape scatter 触发图重编译 + FA tilingkey miss）。
- **`local_experts` MoE decode**：早期实施作为图模式 fixed-shape fallback，Phase 5 MC2 默认后已删。

### 经验教训

- **启动前 `npu-smi info` 看 HBM 均匀**：A3 chip 间歇性 HCCL `RunAicpuKfcResInitV2` AICPU init flaky，被占 chip / firmware 状态污染都会让某些 rank 在 init 阶段挂死。

### 三 exe_mode 性能

| mode | decode (ms/token) |
|---|---:|
| eager | 38.3 |
| ge_graph | 11.1 |
| npugraph_ex | 12.6 |
