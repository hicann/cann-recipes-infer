...
2. 长序列请求的KV Cache内存占用过大，单个rank上存储完整的KV Cache导致device内存out of memory。为缓解由此引入的OOM问题，我们支持了长序列KVP特性：

- 通过使能KVP特性，可实现将长序列的KV Cache沿S维度切分到多个rank上进行存储，从而降低单rank的KV Cache访存压力，KVP切分的大小可通过YAML中的`kvp_size`开关设置。