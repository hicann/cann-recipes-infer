...
class DeepSeekRunner(ModelRunner):
    ...
    def update_model_cfg(self):
        if self.hf_config.compress_ratios is not None:
            # When compress_ratio == 0, set compress_ratio to 1 for convenience.
            for i, val in enumerate(self.hf_config.compress_ratios):
                if val == 0:
                    self.hf_config.compress_ratios[i] = 1
        ...
        for module_name, module in self.model.named_modules():
            ...
            if isinstance(quant_method, QuantizeMethodBase):
                quant_method.process_weights_after_loading(\
                    module, is_nz=is_weight_nz, is_transpose=is_transpose, scales_dtype=scales_dtype)
            ...
            if isinstance(quant_method, CompressedTensorW8A8Int8MoEGMMMethod) or \
                isinstance(quant_method, CompressedTensorW4A8Int8MoEGMMMethod) or \
                isinstance(quant_method, Fp8MoEGMMMethod):
    ...
    def graph_compile(self):
        if not self.enable_cache_compile:
            ...
            if self.execute_mode == "npugraph_ex":
                ...
                self.model.decode = torch.compile(self.model.decode, dynamic=False, fullgraph=True, backend="npugraph_ex", options=compile_options)
            else:
