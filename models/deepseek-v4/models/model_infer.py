...
from executor.utils import get_init_attn_mask, process_infer_time, remove_padding_left, remove_eos_right
from executor.utils.data_utils import tokenizer_in_loop
from .modules import CacheData
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
from utils.encoding_dsv4 import encode_messages, parse_message_from_completion_text
...
def build_dataset_input(tokenizer, prompts, input_max_len, max_new_tokens=32, is_chat=False):
    ...
    if is_chat:
        ...
        _system_prompt_chat = encode_messages(system_prompt_chat, thinking_mode="chat")
        system_prompt_len = len(tokenizer.encode(_system_prompt_chat))
    else:
...
class Infer(nn.Module):
    def __init__(self, runner_settings, main_model, mtp_model=None):
        ...
        self.tmp_cache_data = None
        self.tmp_cache_data_mtp = None
        ...
        self.temperature = runner_settings.get("data_config").get("temperature", 1.0)
    ...
    def inference(self, model_runner, model_inputs, cycle_idx=None, is_prefill=False, warm_up=False, prefix=''):
        ...
        with torch.no_grad():
            if is_prefill:
                ...
                if self.exe_mode == "npugraph_ex" and self.enable_cache_compile and not warm_up:
    ...
    def model_input_prepare(self, model, input_dict):
        ...
        cache_data = input_dict.get("cache_data")
    ...
    def model_output_process(self, model_inputs, outputs, input_dict):
        ...
        input_dict['kv_len'] = model_inputs['attn_metadata']['kv_len'] + 1
        input_dict["cache_data"] = model_inputs.get("cache_data")
        next_tokens = self.sample(outputs, temperature=self.temperature)
    ...
    def model_inference_mtp(self, input_dict_main, input_dict_mtp=None,
        ...
        mtp_argdict['main_next_tokens'] = self.sample(outputs, temperature=self.temperature)
    ...
    def update_model_inputs_prefill(self, model_inputs, input_dict_main, input_dict_mtp,
        ...
        kv_len = model_inputs['attn_metadata']['kv_len'] + 1
        indices = torch.arange(self.spec_len, device="npu")
        kv_len = kv_len.unsqueeze(1) + indices # kv_len increase by one after prefill
    ...
    def update_model_inputs_decode(self, input_dict_main, input_dict_mtp, main_next_tokens,
        ...
        # Reshape operation is used to avoid graph recompilation due to different strides.
    ...
    def mtp_post_process(self, input_dict_mtp, cycle_idx, warm_up, last_prefill=False):
        ...
        for _ in range(loop_mtp):
            ...
            input_dict_mtp = self.mtp_model_output_process(model_inputs, input_dict_mtp, outputs, prev_hidden_states)
    ...
    def mtp_model_output_process(self, model_inputs, input_dict, outputs, prev_hidden_states):
        if input_dict['is_prefill']:
            ...
            kv_len_prefill = model_inputs['attn_metadata']['kv_len']
            indices = torch.arange(self.spec_len, device="npu")
        ...
        # mtp only sample with greedy to verify the correctness of spec tokens, temperature is set to 0.0
        next_tokens = self.sample(outputs, temperature=0.0)
    ...
    def init_cache(self, model, device, input_dict):
        if input_dict['cache_data'] is None:
            cache = CacheData(model.config, model.runner_settings, model.is_mtp, \
                              model.kv_cache_quant_mode, model.li_cache_quant_mode)
            cache_data = cache.init_cache_data(
                num_hidden_layers=1 if model.is_mtp else model.config.num_hidden_layers
            )
            input_dict.update({'cache_data': cache_data})

    def get_inputs(self, prompts, cache_data, cache_data_mtp, warm_up):
        ...
        for prompt in prompts:
            input_prompts.append(encode_messages([{"role":"user", "content": prompt}], thinking_mode="chat"))
        batched_inputs = []
        for prompt in input_prompts:
            batched_inputs.append(tokenizer.encode(prompt, **kwargs))
        input_ids = torch.cat(batched_inputs, dim=0).to(self.main_model.device)
        attention_masks = input_ids != tokenizer.pad_token_id

        input_lens = copy.deepcopy(input_ids.size()[1])
        if not warm_up:
            logging.info(f"Prompt lens is: {input_lens}")
        input_dict_main = {
            "input_ids": input_ids, "generate_ids": input_ids,
        ...
        if self.mtp_model is not None:
            ...
            input_dict_mtp = {
                ...
                "spec_tokens": None,
                "kv_len_cached": None,
                "cache_data": cache_data_mtp
            }
            self.init_cache(self.mtp_model.model, self.mtp_model.device, input_dict_mtp)

        return input_dict_main, input_dict_mtp, input_ids, input_lens
    ...
    def process_mini_batch_inputs(self, input_dict, cycle_idx, is_mtp=False):
        ...
        input_dict_single_cycle['input_lens'] = input_dict['input_lens']
        input_dict_single_cycle['is_prefill'] = True

        # cache_data narrowed
        if self.main_model.prefill_mini_batch_size == 0:
            input_dict_single_cycle['cache_data'] = input_dict['cache_data']
        else:
            input_dict_single_cycle['cache_data'] = ()
            j = cycle_idx % (self.main_model.batch_size_per_rank // self.mini_batch)
            pad_offset = self.main_model.model.attn_metadata.pre_pad_num_block
            if is_batch_required:
                for layer_cache in input_dict['cache_data']:
                    layer_cache_minibatch = {}
                    for cache_type in layer_cache:
                        if layer_cache[cache_type] is not None:
                            batch_len = (layer_cache[cache_type].shape[0] - pad_offset) // \
                                self.main_model.batch_size_per_rank * self.mini_batch
                            layer_cache_minibatch[cache_type] = layer_cache[cache_type].narrow(
                                0, j * batch_len, batch_len + pad_offset)
                        else:
                            layer_cache_minibatch[cache_type] = None
                    input_dict_single_cycle['cache_data'] += (layer_cache_minibatch, )
            else:
                if is_mtp:
                    input_dict_single_cycle['cache_data'] = self.tmp_cache_data_mtp
                else:
                    input_dict_single_cycle['cache_data'] = self.tmp_cache_data
    ...
    def allocate_cp_prefill_multi_cycle_tmpkv(self):
        if self.main_model.cp_size > 1:
            if self.tmp_cache_data is None:
                tmp_cache = CacheData(self.main_model.model.config, self.runner_settings, False,
                                      self.main_model.model.kv_cache_quant_mode,
                                      self.main_model.model.li_cache_quant_mode)
                self.tmp_cache_data = tmp_cache.create_tmp_cache(minibatch=self.mini_batch, 
                                                                 num_hidden_layers=self.main_model.model.config.num_hidden_layers)
                if self.next_n > 0:
                    tmp_cache_mtp = CacheData(self.mtp_model.model.config, self.runner_settings, True,
                                        self.mtp_model.model.kv_cache_quant_mode,
                                        self.mtp_model.model.li_cache_quant_mode)
                    self.tmp_cache_data_mtp = tmp_cache_mtp.create_tmp_cache(minibatch=self.mini_batch, num_hidden_layers=1)
    ...
    def merge_multi_cycle_res(self, input_dict_main_cycles_res, input_dict_mtp_cycles_res,
        ...
        if len(input_dict_main_cycles_res) == 1:
            input_dict_main_cycles_res[0]['cache_data'] = input_dict_main_ori['cache_data']
            if self.mtp_model is not None:
                input_dict_mtp_cycles_res[0]['cache_data'] = input_dict_mtp_ori['cache_data']
        ...
        input_dict_main['cache_data'] = input_dict_main_ori['cache_data']
        input_dict_mtp, mtp_argdict = None, None
        if self.mtp_model is not None:
            input_dict_mtp = self.cat_mini_batch_res(input_dict_mtp_cycles_res)

            input_dict_mtp['cache_data'] = input_dict_mtp_ori['cache_data']
    ...
    def model_generate(self, prompts, cache_data=None, cache_data_mtp=None, warm_up=False):
        # init input_dict and count
        input_dict_main, input_dict_mtp, input_ids, input_lens = self.get_inputs(
                                                            prompts, cache_data, cache_data_mtp, warm_up)
        ...
        with prefill_profiler as prof:
            for _ in range(prefill_prof_cycles):
                ...
                for cycle_idx in range(self.prefill_cycles):
                    is_batch_required = self.cp_size == 1 or \
        ...
        if not warm_up:
            ...
            for generate_ids in generate_ids_list:
                ...
                if self.main_model.tokenizer.eos_token in res:  # call remove_eos_right in future.
