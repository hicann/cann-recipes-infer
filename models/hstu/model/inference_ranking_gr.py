# coding=utf-8
# Adapted from
# https://github.com/NVIDIA/recsys-examples/blob/main/examples/hstu/model/inference_ranking_gr.py
...
def _dense_to_jagged(values: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, int, int]:
    if values.dim() != 3:
        raise ValueError(f"dense values must be 3D, got {values.dim()}D")
    values = values.contiguous()
    batch_size, dense_len, dim = values.shape
    device = values.device
    offsets = torch.arange(batch_size + 1, device=device, dtype=torch.long) * int(dense_len)
    return values.reshape(batch_size * dense_len, dim), offsets, batch_size, dense_len


@torch.fx.wrap
def torch_concat_2d_jagged(
    ...
    def to_jagged_2d(
        values: torch.Tensor, offsets: Optional[torch.Tensor]
    ) -> Tuple[torch.Tensor, torch.Tensor, int, int, bool]:
        if offsets is None:
            v2d, off, batch_size, dense_len = _dense_to_jagged(values)
            return v2d, off, batch_size, dense_len, True

        v2d = values.contiguous()
        off = offsets.to(device=values.device, dtype=torch.long)
        batch_size = int(off.numel() - 1)
        return v2d, off, batch_size, 0, False

    a_is_dense = offsets_a is None
    b_is_dense = offsets_b is None
    if a_is_dense and b_is_dense:
        raise ValueError("torch_concat_2d_jagged does not support dense + dense")

    a_values2d, a_offsets, batch_a, a_dense_len, a_is_dense = to_jagged_2d(values_a, offsets_a)
    b_values2d, b_offsets, batch_b, b_dense_len, b_is_dense = to_jagged_2d(values_b, offsets_b)

    batch_size = batch_a
    dim = int(a_values2d.shape[1])

    a_len = a_offsets[1:] - a_offsets[:-1]  # (B,)
    b_len = b_offsets[1:] - b_offsets[:-1]  # (B,)

    device = a_values2d.device
    dtype = a_values2d.dtype

    def _row_pos_from_offsets(offsets: torch.Tensor, total_len: int):
        if total_len == 0:
            return (
                torch.empty((0,), device=device, dtype=torch.long),
                torch.empty((0,), device=device, dtype=torch.long),
            )
        t = torch.arange(total_len, device=device, dtype=torch.long)
        row = torch.searchsorted(offsets[1:], t, right=True)
        pos = t - offsets[row]
        return row, pos
    if is_replace:
        out2d = a_values2d.clone()
        if a_values2d.numel() == 0:
            return out2d.reshape(batch_size, a_dense_len, dim) if a_is_dense else out2d
        row_b, pos_b = _row_pos_from_offsets(b_offsets, b_values2d.shape[0])
        start_replace = a_offsets[:-1] + (a_len - b_len)
        out_idx_b = start_replace[row_b] + pos_b
        out2d.index_copy_(0, out_idx_b, b_values2d)
        return out2d.reshape(batch_size, a_dense_len, dim) if a_is_dense else out2d

    out_total = int((a_offsets[-1] + b_offsets[-1]).item())
    out2d = torch.empty((out_total, dim), device=device, dtype=dtype)
    if out_total == 0:
        return out2d

    base_out = a_offsets[:-1] + b_offsets[:-1]
    if a_values2d.numel() > 0:
        row_a, pos_a = _row_pos_from_offsets(a_offsets, int(a_values2d.shape[0]))
        out_idx_a = base_out[row_a] + prefix_len + pos_a
        out2d.index_copy_(0, out_idx_a, a_values2d)

    if b_values2d.numel() > 0:
        row_b, pos_b = _row_pos_from_offsets(b_offsets, int(b_values2d.shape[0]))
        la = a_len[row_b]
        if prefix_len == 0:
            out_idx_b = base_out[row_b] + la + pos_b
        else:
            is_pref = pos_b < prefix_len
            out_idx_b = torch.where(
                is_pref,
                base_out[row_b] + pos_b,
                base_out[row_b] + prefix_len + la + (pos_b - prefix_len),
            )
        out2d.index_copy_(0, out_idx_b, b_values2d)
    return out2d
...
class InferenceRankingGR(torch.nn.Module):
    ...
    def __init__(
        ...
        for ebc_config in task_config.embedding_configs:
            if ebc_config.dim != self._embedding_dim:
                raise ValueError("hstu layer hidden size should equal to embedding dim")

        self._embedding_collection = InferenceEmbedding(
            embedding_configs=task_config.embedding_configs,
            hccl_comm_dict=self.hccl_comm_dict,
            embed_tp_size=self.embed_tp_size,
        ...
        self._hstu_block = self._hstu_block.npu()
        self._mlp = self._mlp.npu()
        ...
        self._kvcache_metadata.onload_history_kv_events = [
            torch_npu.npu.Event() for _ in range(hstu_config.num_layers)
        ]
    ...
    def get_user_kvdata_info(
        ...
        return (
            torch.tensor(kvdata_start_pos, dtype=torch.int64),
            torch.tensor(kvdata_lengths, dtype=torch.int64),
        )
