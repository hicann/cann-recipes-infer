...
def unpack_uint4(uint8_data) -> torch.Tensor:
    ...
    if not uint8_data.is_contiguous():
        raise ValueError("uint8_data must be contiguous.")
...
def pack_uint4(uint8_data: torch.Tensor) -> torch.Tensor:
    ...
    shape = uint8_data.shape
    if shape[-1] % 2 != 0:
        raise ValueError(f"The last dimension of shape ({shape[-1]}) must be divisible by 2.")
...
def _reshape_to_blocks(A, axes, block_size):
    ...
    if not all(x >= 0 for x in axes):
        raise ValueError(f"All axes must be non-negative, but got {axes}")
    axes = sorted(axes)
        ...
        for axis in axes:
            ...
            if shape[axis] >= reshape_block_size:
                if shape[axis] % reshape_block_size != 0:
                    raise ValueError(
                        f"Dimension at axis {axis} ({shape[axis]}) must be divisible "
                        f"by reshape_block_size ({reshape_block_size})."
                    )
...
def quantize_mx(
    ...
    if scale_bits <= 0:
        raise ValueError(f"scale_bits must be greater than 0, but got {scale_bits}")
