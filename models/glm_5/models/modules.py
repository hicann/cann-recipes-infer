...
def _init_rope(self):
    # Adapt for different transformer package version
    if self.config.rope_scaling is None or self.config.rope_scaling["rope_type"] == "default":
        self.rotary_emb = DeepseekV3RotaryEmbedding(
            self.config.qk_rope_head_dim,
            max_position_embeddings=self.config.max_position_embeddings,
            base=self.config.rope_theta if hasattr(self.config, 'rope_theta') \
                else self.config.rope_parameters['rope_theta'],
        )
    else:
