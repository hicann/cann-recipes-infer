...
class HunyuanVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        hunyuan_start_time = time.time()
        logger.info('HunyuanVideo start:')
        ...
        logger.info('Text Encoder start:')
        (
            prompt_embeds,
            negative_prompt_embeds,
            prompt_mask,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.autocast(
                    device_type="npu", dtype=target_dtype, enabled=autocast_enabled
                ):
        ...
        if not output_type == "latent":
            ...
            logger.info(f"vae start.") 
            with torch.autocast(
                device_type="npu", dtype=vae_dtype, enabled=vae_autocast_enabled
            ):
                if enable_tiling:
                    self.vae.enable_tiling()
