...
def extract_q_k_double_block_forward(
        self,
        img: torch.Tensor,
        txt: torch.Tensor,
        vec: torch.Tensor,
    ...
    extract_path = os.path.join(os.getenv("EXTRACT_PATH"), f"step-{step_idx}")
    try:
        os.makedirs(extract_path, exist_ok=True)
    except OSError as e:
        logger.info(f"创建目录失败：{e}")

    qk_extract_path = os.path.join(extract_path, f"layer-{layer_idx}-qk.pt")
    qk = {"q": q.cpu(), "k": k.cpu()}
    torch.save(qk, qk_extract_path)
    
    attn = attention(q, k, v, mode='flash', cu_seqlens_q=cu_seqlens_q, 
                    cu_seqlens_kv=cu_seqlens_kv, max_seqlen_q=max_seqlen_q, 
                    max_seqlen_kv=max_seqlen_kv, batch_size=1)
...
def extract_q_k_single_block_forward(
    self,
    x: torch.Tensor,
    vec: torch.Tensor,
    txt_len: int,
