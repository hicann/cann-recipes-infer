...
class Infer(nn.Module):
    def __init__(self, runner_settings, main_model, mtp_model=None):
        ...
        self.li_cache_quant_mode = main_model.model.config.quant_config.li_cache_quant_mode \
            if main_model.model.config.quant_config is not None else "unquant"
    ...
    def model_uses_offload(self, model=None, is_mtp=False):
        if model is not None:
            return bool(getattr(model, "enable_offload", self.enable_offload))
        runner = self.mtp_model if is_mtp else self.main_model
        if runner is not None and getattr(runner, "model", None) is not None:
            return bool(getattr(runner.model, "enable_offload", self.enable_offload))
    ...
    def inference(self, model_runner, model_inputs, cycle_idx=None, is_prefill=False, warm_up=False, prefix=''):
        # MTP IndexShare: prefill/decode return a 4th value `extra` (the MTP layer's top-k, or None for
        # the main model), forwarded as the 4th element of inference()'s return tuple.
        if not warm_up:
            self.logits_wp, self.prev_hidden_states_wp, self.inference_time_wp, self.extra_wp = \
                None, None, None, None
        if warm_up and (cycle_idx is not None):
            if cycle_idx == 0:
                logging.info(f"{model_runner.model_name} [warm up] prefill warm up start ......")
            if cycle_idx > 0:
                return self.logits_wp, self.prev_hidden_states_wp, self.inference_time_wp, self.extra_wp
        ...
        with torch.no_grad():
            if is_prefill:
                logits, prev_hidden_states, extra = model_runner.model.prefill(**model_inputs)
            else:
                if self.exe_mode == "npugraph_ex" and self.enable_skip_guard and not warm_up:
                    with torch.compiler.set_stance(skip_guard_eval_unsafe=True):
                        logits, prev_hidden_states, extra = model_runner.model.decode(**model_inputs)
                else:
                    logits, prev_hidden_states, extra = model_runner.model.decode(**model_inputs)
        ...
        return logits, prev_hidden_states, inference_time, extra
    ...
    def model_input_prepare(self, model, input_dict):
        ...
        offload_cache = None
        if self.model_uses_offload(model):
        ...
        model_inputs = model.prepare_inputs_for_generation(
            ...
            # MTP IndexShare keys; absent for the main model (.get -> None/False = no-op).
            shared_mtp_topk=input_dict.get("shared_mtp_topk"),
            reuse_shared_topk=input_dict.get("reuse_shared_topk", False),
            )
    ...
    def model_inference(self, input_dict, cycle_idx=None, warm_up=False):
        ...
        # MAIN model: extra (4th value) is the MTP-only top-k -> None here, ignored.
        outputs, _, inference_time, _ = self.inference(self.main_model, model_inputs,
    ...
    def model_inference_mtp(self, input_dict_main, input_dict_mtp=None,
        ...
        # MAIN model: extra (4th value) is the MTP-only top-k -> None here, ignored.
        outputs, prev_hidden_states, infer_time_main, _ = self.inference(self.main_model, model_inputs,
    ...
    def mtp_post_process(self, input_dict_mtp, cycle_idx, warm_up, last_prefill=False):
        ...
        # MTP IndexShare (default ON: config.index_share_for_mtp_iteration defaults True, matching upstream
        # GLM-5.2). For next_n>=2 the MTP loop freezes the first decode-step top-k (shared_mtp_topk) and
        # reuses it on later draft steps; the verify firewall keeps decode output byte-identical. Set the
        # config flag to False to disable; getattr default is True so it stays on even if the attr is absent.
        index_share = getattr(self.mtp_model.model.config, "index_share_for_mtp_iteration", True)
        input_dict_mtp['shared_mtp_topk'] = None     # reset at entry (no cross-cycle carry)
        for next_index in range(loop_mtp):
            # Read is_prefill BEFORE mtp_model_output_process (which flips input_dict_mtp['is_prefill'] to
            # False); the reuse/capture gates must observe this step's actual prefill-vs-decode status.
            was_prefill = input_dict_mtp['is_prefill']
            # Reuse the frozen step-0 top-k only on decode draft steps >= 2 (next_index>=1) once captured.
            input_dict_mtp['reuse_shared_topk'] = bool(
                index_share and next_index >= 1 and not was_prefill
                and input_dict_mtp['shared_mtp_topk'] is not None)
            model_inputs = self.model_input_prepare(self.mtp_model.model, input_dict_mtp)
            outputs, prev_hidden_states, infer_time_spec, mtp_topk = self.inference(
                self.mtp_model, model_inputs,
                is_prefill=input_dict_mtp['is_prefill'],
                cycle_idx=cycle_idx,
                warm_up=warm_up,
                prefix='[MTP]')
            infer_time_spec_mtp.append(infer_time_spec)
            # Capture the step-0 (first decode draft step) top-k to freeze for subsequent draft steps.
            should_capture = (index_share and input_dict_mtp['shared_mtp_topk'] is None
                              and not was_prefill and mtp_topk is not None)
            if should_capture:
                input_dict_mtp['shared_mtp_topk'] = mtp_topk.clone()
            # mtp model output process (persists the dict keys; also flips is_prefill -> False)
    ...
    def init_cache(self, model, device, input_dict):
        ...
        if input_dict['past_key_values'] is None:
            if not self.model_uses_offload(model):
                ...
                offload_cache = OffloadCache(getattr(model, "runner_settings", self.runner_settings), model)
    ...
    def get_inputs(self, prompts, past_key_values, past_key_values_indexer,
        ...
        if self.model_uses_offload(self.main_model.model):
        ...
        if self.mtp_model is not None:
            ...
            if self.model_uses_offload(self.mtp_model.model, is_mtp=True):
    ...
    def process_mini_batch_inputs(self, input_dict, cycle_idx, is_mtp=False):
        ...
        if is_batch_required:
            ...
            if self.model_uses_offload(is_mtp=is_mtp) and 'offload_cache' in input_dict:
    ...
    def merge_multi_cycle_res(self, input_dict_main_cycles_res, input_dict_mtp_cycles_res,
        ...
        if len(input_dict_main_cycles_res) == 1:
            if self.model_uses_offload(is_mtp=False) and 'offload_cache' in input_dict_main_ori:
                input_dict_main_cycles_res[0]['offload_cache'] = input_dict_main_ori['offload_cache']
                if self.mtp_model is not None and self.model_uses_offload(is_mtp=True) \
                        and 'offload_cache' in input_dict_mtp_ori:
        ...
        if self.mtp_model is not None:
            ...
            if self.model_uses_offload(is_mtp=True) and 'offload_cache' in input_dict_mtp_ori:
    ...
    def model_generate(self, prompts, past_key_values=None, past_key_values_indexer=None,
        ...
        if self.model_uses_offload(is_mtp=False) and 'offload_cache' in input_dict_main:
            input_dict_main['offload_cache'].reinit_status()
            if self.next_n > 0 and input_dict_mtp is not None and 'offload_cache' in input_dict_mtp:
        ...
        if self.mini_batch and self.next_n > 1:
            logging.info("Mini_batch prefill done, MTP decode begin...")
