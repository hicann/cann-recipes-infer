...
class GlmMoeDsaAttention(nn.Module):
    def __init__(self, config: GlmMoeDsaConfig, runner_settings: Dict, layer_idx: Optional[int] = None,
        ...
        # q_head_dim = 256 for GLM-5.2 (qk_nope 192 + qk_rope 64); 192 in CANN-9.0 compat
        ...
        # GLM-5.2 IndexShare: only "full" layers run their own Lightning Indexer; "shared" layers
        # reuse the previous full layer's top-k (`prev_topk_indices`) and carry no indexer weights.
        # The MTP layer (layer_idx == num_hidden_layers) is always full and is out of range of
        # `indexer_types` (which only covers the main layers), so default it to full.
        indexer_types = getattr(config, "indexer_types", None)
        if layer_idx is not None and indexer_types is not None and layer_idx < config.num_hidden_layers:
            self.skip_topk = indexer_types[layer_idx] == "shared"
        else:
            self.skip_topk = False
        self.indexer = (
            None if self.skip_topk
            else GlmMoeDsaIndexer(self.config, runner_settings, layer_idx, prefix=f"{prefix}.indexer", **kwargs)
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.cp_size > 1 and is_prefill:
            output, topk_indices = self.forward_absorb_cp(**input_kwargs)
            return output.view(batch_size, seq_len, -1), topk_indices
        else:
            input_kwargs.update({"actual_seq_lengths_q": actual_seq_lengths_q})
            output, topk_indices = self.forward_absorb(**input_kwargs)
            return output.view(batch_size, seq_len, -1), topk_indices
    ...
    def forward_absorb(
        ...
        **kwargs,
        ...
        # Return this layer's top-k so the model loop can hand it to the next "shared" layer.
    ...
    def forward_absorb_cp(
        ...
        **kwargs,
        ...
        output = self.mla_epilog(attn_output, absorb=True)
        # `topk_indices` is the (prev, next) tuple; propagate it for the next "shared" CP layer.
        return output, topk_indices
    ...
    def prepare_qkv(
        ...
        **kwargs,
        ...
        # GLM-5.2 MTP IndexShare: a "shared" structural layer (self.skip_topk) reuses the
        # previous full layer's top-k as before. Additionally, when the MTP draft loop requests reuse
        # (reuse_shared_topk=True), a full layer STILL runs its own indexer (it is the only writer of
        # the MTP-private indexer KV cache, so skipping would corrupt the next round) but DISCARDS the
        # recomputed top-k and freezes the captured step-0 selection (prev_topk_indices).
        # Default-off: reuse_shared_topk defaults to False (main path / next_n<2 / prefill never set it).
        reuse_shared_topk = kwargs.get("reuse_shared_topk", False)
        # Invariant: reuse must never co-occur with prefill (else top-k frozen but cache write skipped
        # -> stale indexer cache). Two python bools -> constant-folds to a no-op in compiled graphs.
        assert not (reuse_shared_topk and is_prefill), "MTP IndexShare reuse must not co-occur with is_prefill"
        use_shared = (prev_topk_indices is not None) and (self.skip_topk or reuse_shared_topk)
        if use_shared:
            if reuse_shared_topk and self.indexer is not None and not is_prefill:
                # Keep the indexer KV cache coherent (it is the only writer of the MTP
                # indexer cache) but freeze the selection.
                self.indexer(hidden_states, qr, actual_seq_lengths_kv, kv_len, cos_sin, position_ids, \
                             query_states, \
                             ...
                             is_prefill, cache_only=True)  # write indexer cache, SKIP the lightning-indexer op
            topk_indices = prev_topk_indices
        elif self.indexer is not None:
            # "full" layer: compute this layer's own top-k selection.
            ...
            # "shared" layer: reuse the previous full layer's top-k (a tensor, or a
            # (prev, next) tuple in CP prefill). Must not be None on a shared layer.
...
class GlmMoeDsaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Self Attention (returns this layer's top-k for GLM-5.2 IndexShare propagation)
        ...
        outputs = (residual, hidden_states, topk_indices)
        return outputs
...
class GlmMoeDsaPreTrainedModel(PreTrainedModel):
    ...
    # Set per-instance in GlmMoeDsaForCausalLM.__init__ based on config.num_hidden_layers
    # (the MTP layer index). Main model ignores the MTP layer; the MTP model loads it.
...
class GlmMoeDsaModel(GlmMoeDsaPreTrainedModel):
    ...
    def forward(
        ...
        if self.enable_multi_streams:
            ...
            option = "option_xxx2"
        # GLM-5.2 IndexShare: carry the most recent "full" layer's top-k forward so that
        # "shared" layers can reuse it instead of running their own indexer.
        prev_topk_indices = None
        with superkernel_scope(self.enable_superkernel and not is_prefill, label, option):
            for decoder_layer in self.layers:
                residual, hidden_states, prev_topk_indices = decoder_layer(
                    hidden_states,
                    kv_len,
...
class GlmMoeDsaModelMTPLayer(GlmMoeDsaModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # The MTP layer is a "full" indexer layer (computes its own top-k). With IndexShare,
        # the MTP runner captures this layer's step-0 top-k and feeds it back as `prev_topk_indices` with
        # `reuse_shared_topk=True` on subsequent draft steps; the full indexer still runs (only cache
        # writer) but its return is discarded inside prepare_qkv. We capture & return the MTP layer's own
        # top-k (step-0 produces the to-be-frozen selection). Default-off -> prev_topk_indices=None /
        # reuse_shared_topk=False -> behavior unchanged.
        residual, hidden_states, topk_indices = self.layers[str(self.mtp_start_layer_idx + mtp_layer_idx)](
            hidden_states,
            kv_len,
            actual_seq_lengths_kv,
            cos_sin=cos_sin,
        ...
        return residual, hidden_states, topk_indices
...
class GlmMoeDsaForCausalLM(GlmMoeDsaPreTrainedModel):
    ...
    def __init__(self, config, runner_settings, is_mtp=False, prefix: str = ""):
        ...
        self.is_mtp = is_mtp
        # The MTP layer lives at index == num_hidden_layers (78 at full size, 8 when cropped to 8
        # layers). The main model must ignore it on load; the MTP model loads it. (Was hardcoded 78.)
        self._keys_to_ignore_on_load_unexpected = (
            [] if is_mtp else [rf"model\.layers\.{config.num_hidden_layers}.*"]
        )
        ...
        # Graph-mode IndexShare dispatch handles. Default None: decode() falls back to
        # the single cached_decode handle. Only set to real graphs under the triple condition below.
        self.cached_decode_compute = None
        self.cached_decode_reuse = None
        if self.enable_cache_compile:
            if self.is_mtp and getattr(config, "index_share_for_mtp_iteration", True):
                # GLM-5.2 MTP IndexShare: the per-step python bool `reuse_shared_topk` flips
                # between decode steps; baking it into ONE traced graph would either freeze the first-traced
                # branch (share inert) or trigger guard recompiles. Build TWO static graphs — each freezes a
                # constant branch — and pick by the python bool OUTSIDE the graph (see decode()).
                self.cached_decode_compute = self.get_cached_graph(reuse_variant=False)
                self.cached_decode_reuse = self.get_cached_graph(reuse_variant=True)
            else:
                # Non-MTP, or index_share off: keep the SINGLE graph exactly as before (behavior unchanged).
    ...
    def get_cached_graph(self, reuse_variant=None):
        # graph-mode IndexShare: `reuse_variant` selects which static graph to compile.
        #   None  -> legacy single graph (non-MTP main_decode / MTP main_decode_mtp). DEFAULT-OFF /
        #            non-MTP / index_share=False path.
        #   False -> MTP "compute" graph: cache_model freezes reuse_shared_topk=False (shared_mtp_topk
        #            dropped/None inside the graph). Distinct cache_dir suffix "_compute".
        #   True  -> MTP "reuse"  graph: cache_model freezes reuse_shared_topk=True  (shared_mtp_topk is a
        #            fixed-shape int32 graph INPUT). Distinct cache_dir suffix "_reuse".
        # The two cache_dirs are derived from the SAME stable CASE_NAME env so they are distinct and
        # stable across runs (no on-disk collision between the two graphs).
        ...
        if reuse_variant is True:
            cache_model = self.main_decode_mtp_reuse
            cache_dir = cache_dir + "_reuse"
        elif reuse_variant is False:
            cache_model = self.main_decode_mtp_compute
            cache_dir = cache_dir + "_compute"
    ...
    def prefill(
        ...
        **kwargs
        ...
        # Arity-tolerant unpack: the main model's forward returns a 2-tuple (logits, prev_hidden_states);
        # the MTP override (GlmMoeDsaModelMTP.forward) returns a 3-tuple (..., mtp_topk) for IndexShare.
        # `extra` is None for the main model -> downstream (model_infer.py) ignores it.
        out = self.forward(
            is_prefill=True,
            **kwargs
        )
        logits, prev_hidden_states = out[0], out[1]
        extra = out[2] if len(out) > 2 else None
        return logits, prev_hidden_states, extra
    ...
    def main_decode_mtp_compute(
        self,
        **kwargs
    ):
        # GLM-5.2 MTP IndexShare — graph-mode "compute" variant. This bound method is the
        # cache_compile trace entry for the COMPUTE branch: it FREEZES reuse_shared_topk=False inside the
        # traced graph (a baked-in python constant, NOT a trace-time guard) and DROPS shared_mtp_topk so
        # the tensor is absent (None) inside this graph. Net effect: prepare_qkv's `use_shared` gate is
        # constant-False here -> the full indexer computes & returns its own top-k (mtp_post_process
        # captures it on step-0). The variant pick is a python bool OUTSIDE the graph (in decode()).
        kwargs.pop("reuse_shared_topk", None)
        kwargs.pop("shared_mtp_topk", None)
        out = self.forward(
            is_prefill=False,
            reuse_shared_topk=False,
            shared_mtp_topk=None,
            **kwargs
        )
        logits, prev_hidden_states = out[0], out[1]
        extra = out[2] if len(out) > 2 else None
    ...
    def main_decode_mtp_reuse(
        self,
        **kwargs
    ):
        # GLM-5.2 MTP IndexShare — graph-mode "reuse" variant. This bound method is the
        # cache_compile trace entry for the REUSE branch: it FREEZES reuse_shared_topk=True inside the
        # traced graph (a baked-in python constant, NOT a trace-time guard) and KEEPS shared_mtp_topk as a
        # graph INPUT (a fixed-shape int32 tensor, never None inside this graph). Net effect: prepare_qkv's
        # `use_shared` gate is constant-True here -> the full indexer STILL runs (cache coherent) but
        # its recomputed top-k is discarded and the frozen shared_mtp_topk selection is used. The variant
        # pick is a python bool OUTSIDE the graph (in decode()).
        kwargs.pop("reuse_shared_topk", None)
        out = self.forward(
            is_prefill=False,
            reuse_shared_topk=True,
            **kwargs
        )
        logits, prev_hidden_states = out[0], out[1]
        extra = out[2] if len(out) > 2 else None
    ...
    def decode(
        ...
        **kwargs
        ...
        if self.enable_cache_compile:
            # MTP IndexShare: when the reuse/compute static graphs exist (MTP + cache_compile +
            # index_share_for_mtp_iteration), pick by the per-step python bool reuse_shared_topk outside
            # the graph (each graph bakes the branch as a constant, so neither recompiles). Otherwise use
            # the single cached_decode handle.
            if getattr(self, "cached_decode_reuse", None) is not None:
                _reuse = kwargs.get("reuse_shared_topk", False)
                if _reuse:
                    _smt = kwargs.get("shared_mtp_topk")
                    # reuse graph requires the captured step-0 top-k (shape/dtype check only, no device sync).
                    assert (_smt is not None and _smt.dtype == torch.int32
                            and _smt.shape[-1] == self.config.index_topk), \
                        "reuse graph requires captured int32 [*, index_topk] shared_mtp_topk"
                graph = self.cached_decode_reuse if _reuse else self.cached_decode_compute
                out = graph(**kwargs)
            else:
                out = self.cached_decode(**kwargs)
        else:
            out = self.main_decode(**kwargs)
        logits, prev_hidden_states = out[0], out[1]
        extra = out[2] if len(out) > 2 else None
        return logits, prev_hidden_states, extra
    ...
    def init_cache_for_indexer(
        ...
        # IndexShare: only "full" layers hold an indexer (shared layers have indexer=None and never
        # access their cache slot), so allocate one indexer KV-cache slot per full layer instead of per
        # layer. MTP model layers are all full; GLM-5 (no indexer_types) falls back to per-layer.
        indexer_types = getattr(self.config, "indexer_types", None)
        if self.is_mtp or indexer_types is None:
            num_indexer_slots = num_hidden_layers
        else:
            num_indexer_slots = sum(1 for t in indexer_types[:num_hidden_layers] if t == "full")

        for _ in range(num_indexer_slots):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs
        ...
        model_inputs = {
            ...
            "slot_mapping": slot_mapping,
            # GLM-5.2 MTP IndexShare: threaded only for the MTP model. The main model never
            # sets these keys, so .get(...) yields None/False here -> consumed as no-op in forward.
            "shared_mtp_topk": kwargs.get("shared_mtp_topk"),
            "reuse_shared_topk": kwargs.get("reuse_shared_topk", False),
        }
        if self.enable_offload:
...
class GlmMoeDsaModelMTP(GlmMoeDsaForCausalLM):
    ...
    def forward(
        ...
        **kwargs
        ...
        # GLM-5.2 MTP IndexShare: the runner threads the captured step-0 top-k via
        # `shared_mtp_topk` and the per-step `reuse_shared_topk` flag through kwargs. Default-off:
        # main model / next_n<2 never set these -> shared=None, reuse=False -> no behavior change.
        shared = kwargs.get("shared_mtp_topk")
        reuse = kwargs.get("reuse_shared_topk", False)

        residual, hidden_states, mtp_topk = self.model(
            hidden_states,
            kv_len,
            actual_seq_lengths_kv,
            actual_seq_lengths_q=actual_seq_lengths_q,
        ...
        return logits, prev_hidden_states, mtp_topk
